...
## 2026-06-06 AndrewsForest 预热期数据可用性检查

- 按用户要求检查当前数据是否支持把 storm 模拟开始时间往前推，以便在正式评估窗口前包含 1-2 段完整降雨事件。
- 本地 CSV 数据范围：
  - `pcp_Intensity_5min.csv`：`2006-01-01 00:00:00` 至 `2017-01-01 00:00:00`，5 分钟步长；
  - `meteo_storm.csv`：`2015-01-01 00:00:00` 至 `2015-12-31 23:55:00`，5 分钟步长；
  - `observed_Q_storm_outlet.csv`：`2015-01-01 00:00:00` 至 `2015-04-24 13:10:00`，5 分钟步长。
- MongoDB `andrews_forest_HydroClimate` 数据范围：
  - 降雨 `P` 覆盖 `2006-01-01 00:00:00` 至 `2017-01-01 00:00:00`，`TIMESTEP=300`；
  - storm 气象变量 `TMEAN/TMAX/TMIN/RM/WS/SR` 均覆盖 `2015-01-01 00:00:00` 至 `2015-12-31 23:55:00`，`TIMESTEP=300`；
  - 出口实测流量 `Q` 覆盖 `2015-01-01 00:00:00` 至 `2015-04-24 13:10:00`，`TIMESTEP=300`。
- 事件识别方法：
  - 以 12 小时无雨间隔划分 storm 降雨事件；
  - `pcp_Intensity_5min.csv` 输入值按代码说明为 `mm/h` 强度，插值后输出每步雨深，因此事件累计雨量按 `强度 * 300 / 3600` 换算。
- 当前开始时间 `2015-02-04 06:00:00` 之前的主要事件：
  - `2015-02-01 10:10:00` 至 `2015-02-04 05:30:00`：完整降雨事件，前一段无雨约 `64.33 h`，等效平均雨深约 `49.94 mm`；
  - `2015-01-17 03:35:00` 至 `2015-01-19 10:10:00`：完整降雨事件，等效平均雨深约 `83.13 mm`；
  - `2015-01-15 16:35:00` 至 `2015-01-16 10:45:00`：完整降雨事件，等效平均雨深约 `30.30 mm`。
- 预热期建议：
  - 最小可行方案：把 `STARTTIME` 改为 `2015-02-01 00:00:00`，能包含一段完整且紧贴评估期的预热降雨事件；
  - 更推荐方案：把 `STARTTIME` 改为 `2015-01-15 00:00:00`，能包含 1 月 15-19 日的两段显著降雨响应，以及 2 月 1-4 日的完整降雨事件；
  - 更稳妥但稍长方案：`2015-01-14 00:00:00`，在 1 月 15 日事件前留出约 1 天初始稳定时间。
- 当前判断：
  - 数据层面完全支持把模拟开始时间往前推；
...
## 2026-06-06 2 月 9-10 日产流过强原因诊断

- 按用户要求检查 warmup 后 2 月 9-10 日产流过强原因，使用同一时间窗口重新运行诊断版模型：
  - `SEIMS_WB_DIAG=1` 输出 `SUR_SGA_balance.csv`、`DEP_FS_balance.csv`、`IKW_OL_balance.csv`；
  - `SEIMS_IKW_CH_DIAG=1` 输出 `IKW_CH_diag.csv`；
  - 诊断结果归档到 `data/AndrewsForest/andrews_forest_model/storm/experiments/20260606_warmup_diag_ch_20150115_20150216/`。
- 新增诊断文档：
  - `data/AndrewsForest/andrews_forest_model/storm/feb09_10_warmup_overrun_diagnosis.md`。
- 降雨单位复核：
  - 2 月 9-10 日 `pcp_Intensity_5min.csv` 原始均值累计 `577.92`；
  - 若按 `mm/h` 强度积分，对应雨深 `48.16 mm`；
  - `SUR_SGA` 实际接收净雨约 `44.17 mm`，说明当前模型输入到入渗模块的雨量已经是每步雨深量级，并非把 `577.92 mm/h` 直接当作 `577.92 mm`。
- 2 月 9-10 日水量路径：
  - `SUR_SGA` 净雨 `44.17 mm`，入渗仅 `0.035 mm`；
  - `DEP_FS` 新增地表水 `44.14 mm`，几乎等于全部新净雨；
  - `IKW_OL` 原始入河 `QS` `37.41 mm`，实际二次下渗仅 `0.021 mm`；
  - `IKW_CH` 侧向入流中 `QS` 占约 `94.5%`，`QI` 约 `1.5%`，`QG` 约 `3.4%`。
- 根因判断：
  - 2 月 9-10 日异常峰不是壤中流或地下水主导，也不是降雨单位放大主导；
  - 直接原因是预热期和 2 月 7 日事件后，`SUR_SGA` 活动层入渗容量几乎耗尽，`cap_zero_cells` 最大达到 `68399 / 68406`，`positive_infil_cells` 平均只有约 `32` 个；
  - `m_accumuDepth` 平均约 `51.85 mm`，GAR 恢复在该事件内仅约 `0.045 mm`，未能在 2 月 7 日后至 2 月 9 日前的无雨期恢复有效入渗容量；
  - 高 `MANNING=8` 造成坡面水先滞留、再延迟释放：在模拟峰值 `2015-02-10 14:05:00` 时已经无新降雨，但非河道坡面仍有约 `26.25 mm` 地表水，河道格约 `2.17 mm`。
- 额外发现：
  - 当前 `param.cali` 的 `CH_N=0.85,VC` 没有生效；运行日志提示 `Load parameter: CH_N failed! We will ignore it!`，代码检查显示 `IKW_CH` 没有把 `CH_N` 注册为可校准参数，河道糙率主要来自 `REACHES`。
- 下一步建议：
  - 保持 `2015-01-15` 预热窗口；
...
## 2026-06-13 GW0 与 KG 的代码含义及物理合理性说明

- `GW0` 在 `GW_RSVR` 中对应 `m_initStorage`，单位为 `mm`；模块第一次初始化时，把每个子流域地下水概念水库的 `m_storage` 统一设为该值。
- `GW0` 是折算到子流域面积上的地下水可排泄储量水深，不是地下水位埋深，也不是含水层实际厚度。`GW0=220 mm` 表示初始可动地下水储量相当于流域面上 `0.22 m` 水层。
- 当前 `GWMAX=300 mm`，因此 `GW0=220 mm` 占概念水库最大容量约 `73.3%`，没有触发库容上限。从概念水库数值范围看可以接受，但仅凭流量率定不能证明真实流域初始储量恰好为 220 mm。
- `KG` 在代码中对应 `m_recessionCoefficient`，实际出流公式为 `outFlowDepth = storage * (1 - exp(-KG * dt_days))`，因此单位应解释为 `d-1`，虽然模块元数据目前写成无量纲。
- `KG=0.05 d-1` 表示无补给条件下每天释放当前储量约 `4.88%`，指数衰减特征时间约 `20 d`，半衰期约 `13.86 d`。初始 `220 mm` 对应首日理论释放约 `10.73 mm`，之后随储量递减，并叠加 `PERCO_DARCY` 的地下水补给。
- 物理判断：`220 mm` 对森林坡地的土壤-风化层-浅层基岩综合可动储量并非数量级上不可能，且当前实验中能把正式评价期前的慢流背景提高到约 `2.02 m3/s`；但它较接近 `GWMAX`，仍应视为事件模型缺少更长预热和空间化地下水初值时的有效概念初值，而不是已经被观测独立验证的含水层参数。
- 后续验证应优先用事件前退水曲线估计 `KG`，再由观测基流反推 `GW0`；若不同事件需要显著不同的 `GW0`，应增加分子流域/空间化地下水初值或延长预热，而不是继续把单一 `GW0` 当作固定流域属性。

## 2026-06-13 KI 未实现 QS 向 QI 重分配的原因与下一步计划

- 当前模块执行顺序为 `SUR_SGA -> DEP_FS -> IKW_OL -> IKW_IF -> PERCO_DARCY`。同一时间步中，`SUR_SGA` 已先决定入渗和超渗水，`DEP_FS/IKW_OL` 已生成并汇流地表水，之后 `IKW_IF` 才按 `KI` 排出土壤中已有的水。因此增大 `KI` 不能追溯性地把当步已生成的 `EXCP/SURU/QS` 改成 `QI`。
- `KI` 只放大高于田间持水量部分的矩阵壤中流和大孔隙流；它不参与 `SUR_SGA` 的入渗公式，也不直接修改 `m_accumuDepth`。
- 实验结果验证了这一点：`KI` 从 `0.007` 增至 `0.014/0.021` 后，2 月 10 日平均 `QI` 从约 `0.55` 增至 `0.84/1.02 m3/s`，但平均 `QS` 基本保持约 `4.66 m3/s`，所以总峰值反而由 `11.87` 增至 `12.26/12.48 m3/s`。
- “2 月 7 日后土壤接近满”应更准确地解释为：固定约 `150 mm` 活动层的孔隙容量与 Green-Ampt 事件累计入渗记忆接近上限；并不表示整个土壤剖面或深层地下水库都已饱和。`IKW_IF/PERCO_DARCY` 能排水，但当前 `SUR_SGA` 只通过自身 dry-period active-drain 显式降低 `m_accumuDepth`，没有按实际壤中流/渗漏造成的活动层储量下降同步恢复事件入渗容量。
- 下一步修改计划：
  1. 增加活动层闭合诊断，逐步输出活动层实际储水、对孔隙度亏缺、对初始状态新增水量、`m_accumuDepth`、`IKW_IF` 各层排水、`PERCO_DARCY` 各层下渗及坡面存水；分别汇总 2 月 7 日前、2 月 7 日事件后无雨期和 2 月 9-10 日事件；
  2. 将 Green-Ampt 记忆恢复改为“真实排水驱动”：记录上一时刻活动层事件水，只有当无雨期活动层储水因 `IKW_IF/PERCO_DARCY` 实际减少时，才按对应水深降低 `m_accumuDepth`；不再主要依靠固定 `mm/h` 速率凭时间恢复，也不由 `SUR_SGA` 额外搬运大量水到深层；
  3. 把 `ACTIVE_DEPTH_MAX` 从固定活动层深度改为湿润锋最大深度上限，湿润锋随累计入渗和各层含水亏缺逐层推进，使 2 月 7 日后仍可能利用更深未饱和层，而不是固定 150 mm 容量耗尽后把 2 月 10 日降雨几乎全部转为地表水；
  4. 在水量路径改善后再测试 `IF_SUBSTEPS=2/4`，检查坡面壤中流能否在 2 月 7 日事件及其退水段到达河道；暂不继续增加 `KI/ANISOTROPY/MACROPORE_FACTOR`；
  5. 最后仅用小范围 `MANNING=6-8` 修正峰宽和峰现，不再用 Manning 解决产流水量分配。
- 目标约束：保持雨前流量约 `2 m3/s`；2 月 7 日峰值由当前 `7.42` 提高到约 `9-10.5 m3/s`；2 月 10 日峰值由 `11.87` 降至约 `8-9.5 m3/s`；2 月 10 日 `QS` 体积占比由约 `70%` 降到 `55%-60%` 以下，同时逐步水量闭合。

## 2026-06-13 SUR_SGA 恢复、旧坡面水二次下渗与 IKW_IF 子步实验

- 按上一节计划进行了三类机制实验，模拟窗口保持 `2015-01-15 00:00:00` 至 `2015-02-16 12:00:00`，正式评价窗口为 `2015-02-04 06:00:00` 至 `2015-02-16 12:00:00`。实验结果归档在 `data/AndrewsForest/andrews_forest_model/storm/experiments/20260613_state_coupled_recovery/`。
- 真实排水驱动的 `m_accumuDepth` 恢复实验：
  - 思路是记录上一时刻活动层相对初始状态的事件水量，只有无雨期实际活动层水量下降时，才同步降低 Green-Ampt 的累计入渗记忆；
  - 结果显示该恢复量相对 active-drain 很小，2 月 10 日峰值仅从约 `11.87` 降到 `11.79 m3/s`，但 2 月 7 日峰值也略降，正式期 NSE 从 `0.0303` 变为约 `0.0227`；
  - 判断：机制方向可以解释“土壤排水应恢复入渗容量”，但在当前模块耦合下贡献太弱，不能作为有效代码修改提交。
- 旧坡面水 `SURU` 回到 `SUR_SGA` 二次下渗实验：
  - 该实验把上一时刻残留坡面水计入本步 `hWater`，并在进入 `DEP_FS` 前清空旧 `SURU`，避免双重计水；
  - 结果正式期 NSE 提高到约 `0.1146`，但目标问题没有改善：2 月 7 日峰值降为约 `6.37 m3/s`，2 月 10 日峰值仍约 `11.94 m3/s`；
  - 诊断说明旧坡面水确实回到 `SUR_SGA`，但事件期入渗容量常接近零，旧水大多又变成 `EXCP/SURU`，没有真正被土壤消化；
  - 判断：与用户设想的水量路径一致，但对目标峰值顺序有负面影响，因此不保留。
- 动态湿润锋 + `ACTIVE_DEPTH_MAX=300 mm` 实验：
  - 试图把 `ACTIVE_DEPTH_MAX` 解释为湿润锋最大深度，使深层未饱和容量参与入渗；
  - 结果过强，2 月 7 日几乎全部入渗，2 月 7 峰值只剩约 `2.54 m3/s`，2 月 10 也被压到约 `3.01 m3/s`，正式期 NSE 约 `-0.9818`；
  - 判断：深层容量不能在降雨步瞬时全部开放；若要使用更深湿润锋，必须限制为无雨间歇期的再分布/恢复，而不是直接增加当步入渗容量。
- `IKW_IF` 壤中流子步实验：
  - 临时加入 `IF_SUBSTEPS=4`，测试是否因为壤中流汇流太慢导致 2 月 7 日流量不足；
  - 结果 2 月 7 峰值约 `6.24 m3/s`，2 月 10 峰值约 `11.79 m3/s`，正式期 NSE 约 `0.0220`；
  - 判断：增加壤中流数值子步没有把 2 月 7 峰值抬高，也没有显著压低 2 月 10 峰值，说明“壤中流到河道太慢”不是当前主要矛盾。
- 处理结果：
  - 上述实验均未达到“2 月 7 日峰值升高、2 月 10 日峰值降低”的目标，因此没有提交代码修改；
  - 已从 `param.cali` 和 MongoDB `PARAMETERS_SPEC` 中移除临时 `IF_SUBSTEPS=4`，并重新编译同步 `SUR_SGA` 动态库；
  - 当前 `storm/OUTPUT_D8_DOWNUP--/Q.txt` 已恢复为当前稳定参数运行结果，正式期 NSE `0.0303`、PBIAS `-23.08%`，2 月 10 峰值仍为 `11.87 m3/s`。
- 下一步建议：
...
## 2026-06-16 IKW_CH 子步体积修复后的完整窗口运行

- 按用户要求用修复后的 `IKW_CH` 重新运行完整窗口：`2015-01-15 00:00:00` 至 `2015-02-16 12:00:00`。
- 运行命令：
  - `python run_andrews_storm.py --start '2015-01-15 00:00:00' --end '2015-02-16 12:00:00' --figure-name q_pcp_comparison_ikw_ch_substep_fix_full.png`
- 输出：
  - `data/AndrewsForest/andrews_forest_model/storm/OUTPUT_D8_DOWNUP--/Q.txt`
  - `data/AndrewsForest/andrews_forest_model/storm/OUTPUT_D8_DOWNUP--/q_pcp_comparison_ikw_ch_substep_fix_full.png`
  - 另生成正式评价窗口图：`q_pcp_comparison_ikw_ch_substep_fix_formal.png`
- 全窗口 `2015-01-15` 至 `2015-02-16`：
  - NSE `-1.2351`，PBIAS `67.49%`；
  - 模拟均流 `6.901 m3/s`，实测均流 `4.120 m3/s`；
  - 模拟峰值 `23.903 m3/s`，发生在 `2015-02-10 15:30:00`；实测全窗口峰值 `15.886 m3/s`，发生在 `2015-01-18 04:00:00`。
- 正式评价窗口 `2015-02-04 06:00:00` 至 `2015-02-16 12:00:00`：
  - NSE `-3.7887`，PBIAS `53.40%`；
  - 模拟均流 `7.329 m3/s`，实测均流 `4.777 m3/s`；
  - 模拟峰值 `23.903 m3/s`，发生在 `2015-02-10 15:30:00`；实测峰值 `10.449 m3/s`，发生在 `2015-02-07 15:00:00`。
- 2 月 7 日事件：
  - NSE `-0.1209`，PBIAS `-12.49%`；
  - 模拟峰值 `13.668 m3/s`，实测峰值 `10.449 m3/s`，峰现滞后 `9 h`。
- 2 月 10 日事件：
  - NSE `-984.8008`，PBIAS `139.22%`；
  - 模拟峰值 `23.903 m3/s`，实测峰值 `8.382 m3/s`，峰现滞后 `10.75 h`。
- 解释：
  - `IKW_CH` 子步体积修复后，河道接收到的 `QS/QI/QG` 不再被减半，因此当前旧参数组合会明显高估慢流背景和 2 月 10 日洪峰；
  - 这说明此前 `GW0=220`、`KG=0.05`、较强 `IKW_IF` 参数和当前汇流糙率是在“通道侧向入流偏小”的数值状态下调出来的，修复后必须重新率定；
  - 下一步应先降低/重估 `GW0` 或 `KG` 以恢复合理基流，再检查 2 月 10 日峰值是否仍由坡面 `QS` 过强或坡面水滞留释放造成，最后再调 `MANNING/CH_N`。

## 2026-06-16 IKW_CH 修复后重新率定 GW0/KG

- 按用户要求重新降低/率定 `GW0/KG`，所有候选均运行完整窗口 `2015-01-15 00:00:00` 至 `2015-02-16 12:00:00`，结果图包含 1 月预热期。
- 实验归档目录：`data/AndrewsForest/andrews_forest_model/storm/experiments/20260616_gw0_kg_after_ikw_ch_fix/`，汇总表为 `summary.csv`。
- 候选结果：
  - `GW0=0, KG=0.05`：正式期 NSE `-2.4039`，PBIAS `8.36%`，2 月 4-6 日均流 `1.323 m3/s`，2 月 7 日峰值 `11.127 m3/s`，2 月 10 日峰值 `21.817 m3/s`；
  - `GW0=40, KG=0.05`：正式期 NSE `-2.4855`，PBIAS `16.42%`，2 月 4-6 日均流 `1.808 m3/s`，2 月 7 日峰值 `11.584 m3/s`，2 月 10 日峰值 `22.189 m3/s`；
  - `GW0=60, KG=0.04`：正式期 NSE `-2.5214`，PBIAS `18.56%`，2 月 4-6 日均流 `1.991 m3/s`，2 月 7 日峰值 `11.740 m3/s`，2 月 10 日峰值 `22.292 m3/s`；
  - `GW0=80, KG=0.03`：正式期 NSE `-2.5449`，PBIAS `19.62%`，2 月 4-6 日均流 `2.104 m3/s`，2 月 7 日峰值 `11.834 m3/s`，2 月 10 日峰值 `22.347 m3/s`；
  - `GW0=120, KG=0.02`：正式期 NSE `-2.5884`，PBIAS `21.45%`，2 月 4-6 日均流 `2.250 m3/s`，2 月 7 日峰值 `11.967 m3/s`，2 月 10 日峰值 `22.442 m3/s`；
  - `GW0=160, KG=0.015`：正式期 NSE `-2.6212`，PBIAS `22.75%`，2 月 4-6 日均流 `2.339 m3/s`，2 月 7 日峰值 `12.053 m3/s`，2 月 10 日峰值 `22.508 m3/s`。
- 当前选择：
  - 将 `param.cali` 更新为 `GW0=60, KG=0.04`；
  - 该组 2 月 4-6 日模拟均流约 `1.99 m3/s`，最接近用户提出的约 `2 m3/s` 稳定基流；
  - `GW0=0, KG=0.05` 虽然正式期 PBIAS 最低，但相当于清空初始地下水储量，物理上只能作为诊断下限，不适合作为最终基线。
- 当前主输出目录为 `GW0=60, KG=0.04` 的完整窗口结果：
  - `OUTPUT_D8_DOWNUP--/Q.txt`
  - `OUTPUT_D8_DOWNUP--/q_pcp_comparison_gw0_60_kg_0p04_full.png`
- 结论：
  - 降低 `GW0/KG` 能把基流和正式期总水量从修复后的严重偏高状态拉回一部分；
  - 但 2 月 10 日峰值在所有候选中仍约 `21.8-22.5 m3/s`，远高于实测 `8.382 m3/s`，说明第二场过峰不是初始地下水单独造成；
  - 下一步应重点诊断/调整 2 月 7 日后到 2 月 10 日前的坡面水 `SURU/m_sr` 滞留释放、`DEP_FS/IKW_OL` 入河连通，以及 `SUR_SGA` 在第二场雨前是否恢复了足够入渗容量。

## 2026-06-16 GW0/KG 新基线下的 SUR_SGA-DEP_FS-IKW_OL 连通诊断

- 按下一步计划，用当前新基线 `GW0=60, KG=0.04` 重新运行完整窗口 `2015-01-15 00:00:00` 至 `2015-02-16 12:00:00`，并开启：
  - `SEIMS_WB_DIAG=1`
  - `SEIMS_IKW_CH_DIAG=1`
  - `SEIMS_IKW_IF_DIAG=1`
- 输出图：`OUTPUT_D8_DOWNUP--/q_pcp_comparison_gw0_60_kg_0p04_full_diag.png`。
- 当前评价结果：
  - 正式期 NSE `-2.5214`，PBIAS `18.56%`；
  - 2 月 4-6 日均流：模拟 `1.991 m3/s`，实测 `2.683 m3/s`；
  - 2 月 7 日峰值：模拟 `11.740 m3/s`，实测 `10.449 m3/s`，但峰现滞后 `9 h`；
  - 2 月 10 日峰值：模拟 `22.292 m3/s`，实测 `8.382 m3/s`，峰现滞后 `10.83 h`。
- 水量路径诊断：
  - 2 月 7 日事件期：`SUR_SGA` 的 `hWater` 约 `1.34e7 m3`，其中新净雨约 `2.03e6 m3`，入渗仅约 `3.01e5 m3`，`EXCP` 约 `1.31e7 m3`，`EXCP/hWater=0.978`；
  - 2 月 10 日事件期：新净雨只有约 `4.80e5 m3`，但 `hWater` 约 `8.80e6 m3`，入渗仅约 `4.87e4 m3`，`EXCP` 约 `8.76e6 m3`，`EXCP/hWater=0.994`；
  - 2 月 10 事件期的 `hWater` 主要来自旧洼蓄 `DPST`，而不是新雨；同一时段 `surf_old/SURU` 已经是约 `4.97e8 m3` 的时间累计量，但当前 `SUR_SGA` 计算 `hWater` 时没有把旧坡面水直接加入，只由 `IKW_OL` 继续处理。
- `SUR_SGA` 入渗容量状态：
  - 2 月 7 日事件平均 `cap_zero_cells` 约 `5.48e4` 个，2 月 10 日事件平均约 `3.72e4` 个；
  - 在 2 月 10 日 03:15，`cap_zero_cells=68394`，`positive_infil_cells=12`，全流域入渗仅约 `3.4 m3/step`，几乎所有可用水都转为 `EXCP`；
  - 2 月 8-9 日间歇期确实有恢复：`accumu_recovery` 约 `5.28e5 m3`，但同期 `hWater` 仍约 `1.59e7 m3`，`EXCP` 约 `1.56e7 m3`，说明恢复量远不足以打开第二场雨前关键湿润格子的入渗容量。
- `DEP_FS` 与坡面水：
  - 2 月 7 日事件中 `DEP_FS` 新增坡面径流约 `1.72e6 m3`，约为 `EXCP` 的 `13.1%`；
  - 2 月 10 日事件中 `DEP_FS` 新增坡面径流约 `4.73e5 m3`，约为 `EXCP` 的 `5.4%`；
  - 但 `m_sr/SURU` 状态在事件前已累积很大，2 月 10 日 03:15 的 `sr_new` 状态约 `2.31e6 m3`，导致后续 `IKW_OL` 持续向河道释放。
- `IKW_OL/IKW_CH` 连通：
  - 2 月 10 事件期 `IKW_OL` 坡面总出流约 `6.03e7 m3`，stream cell 原始 `QS` 约 `1.43e6 m3`，只占总坡面出流约 `2.4%`；
  - `IKW_CH` 现在接收到的 `QS` 与 `IKW_OL` stream cell 原始 `QS` 比例为 `1.0`，说明前一轮子步体积缩小问题已经修复；
  - 2 月 10 的 `CH` 侧向输入以 `QS` 为主，事件期 `QS≈1.43e6 m3`，`QI≈9.25e4 m3`，`QG≈1.04e5 m3`，因此当前过峰主要是坡面快流/坡面水状态问题，不是地下水或壤中流主导。
- 二次下渗诊断：
  - 2 月 10 事件期 `IKW_OL` 潜在二次下渗约 `2.57e6 m3`，实际二次下渗约 `2.01e5 m3`，仅占潜在量约 `7.8%`，也只占坡面出流约 `0.33%`；
  - 但 `unused_capacity_with_water` 仅约 `803 m3`，`final_surface_with_unused_capacity` 为 `0`，说明未用入渗容量多数出现在没有坡面水的格子；有坡面水的格子上局地入渗容量基本已经被关掉。
- 当前判断：
...
## 2026-06-16 m_sr/SURU 长期滞留与 IKW_OL 入河滞后排查

- 本次只排查，不修改代码。
- 代码链路：
  - `DEP_FS` 中 `m_sr` 是坡面积水状态量，`currentRunoff` 由本步 `EXCP` 填满洼蓄后的剩余量得到，然后执行 `m_sr[i] += currentRunoff`；
  - `IKW_OL` 每步把 `m_sr` 转为水深 `h`，用运动波公式计算出流 `m_qs`，未出流部分重新写回 `m_sr`；
  - `IKW_CH` 只接收 stream cell 上的 `m_qs[id][0]` 作为侧向入流 `QS`，因此从新增坡面水到出口流量的滞后可拆成 `DEP_FS -> IKW_OL stream_raw` 和 `IKW_CH -> Q.txt` 两段。
- 峰现拆分：
  - 1 月 18 日预热期：实测峰 `2015-01-18 04:00`，`IKW_OL stream_raw` 峰 `2015-01-18 12:50`，出口模拟峰 `2015-01-18 16:50`；总滞后约 `12.83 h`，其中坡面入河约 `8.83 h`，河道约 `4 h`；
  - 2 月 7 日：实测峰 `2015-02-07 15:00`，`IKW_OL stream_raw` 峰 `2015-02-08 03:30`，出口模拟峰 `2015-02-08 04:55`；总滞后约 `13.92 h`，其中坡面入河约 `12.5 h`，河道约 `1.42 h`；
  - 2 月 10 日：实测峰 `2015-02-10 04:45`，`IKW_OL stream_raw` 峰 `2015-02-10 12:10`，出口模拟峰 `2015-02-10 15:30`；总滞后约 `10.75 h`，其中坡面入河约 `7.42 h`，河道约 `3.33 h`。
- 2 月 10 日过峰量级：
  - 2 月 10 日事件新增 `DEP_FS runoff_added` 约 `5.25e5 mm-cell`，折合约 `4.72e5 m3`；
  - 同期 `IKW_OL stream_raw` 入河约 `1.87e6 m3`，约为新增坡面水的 `4` 倍，说明 2 月 10 的 `QS` 主要来自前期 `m_sr/SURU` 库存释放，不是本场新产流本身；
  - 2 月 10 日 03:15 前后 `m_sr` 状态达到约 `2.26e6 m3`，到 12:10 仍有约 `1.70e6 m3`，因此旧坡面水库存充足且释放滞后。
- 当前最可能原因：
  - `param.cali` 中 `MANNING=8.0,VC` 已写入 MongoDB；代码中 `VC` 是绝对赋值，不是乘子，因此坡面曼宁糙率被统一设成 `8`，这是非常大的物理糙率，会显著拖慢坡面运动波；
  - `IKW_OL` 对 stream cell 没有“到达河道即侧向入河”的单独机制，而是让水继续按栅格运动波通过 stream cell 后，才以 `m_qs[id][0]` 进入 `IKW_CH`；这会给坡面到河道多加一个 stream-cell 路由滞后；
  - 当前 `IKW_OL` 二次下渗受局地入渗容量限制，诊断中有水格子的未用容量很小，不能显著消耗旧 `m_sr`；
  - `CH_N=0.85,VC` 也偏大，会增加 1-4 小时河道段滞后，但不是主延迟。
- 下一步计划：
  - 先做参数语义控制实验：把 `MANNING` 从绝对 `VC=8` 改为使用原空间糙率或较小的相对系数 `RC`，重新跑完整窗口，验证滞后是否明显缩短；
