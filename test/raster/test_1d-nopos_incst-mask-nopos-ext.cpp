...
class clsRasterDataTestNoPosIncstMaskNoPosExt: public TestWithParam<InputRasterFiles *> {
public:
    clsRasterDataTestNoPosIncstMaskNoPosExt() : rs_(nullptr), maskrs_(nullptr) {
    }

    virtual ~clsRasterDataTestNoPosIncstMaskNoPosExt() { delete rs_; }
...
TEST_P(clsRasterDataTestNoPosIncstMaskNoPosExt, RasterIO) {
    EXPECT_TRUE(maskrs_->PositionsCalculated()); // When use mask's extent, positions will be calculated.
    ...
    EXPECT_FLOAT_EQ(19., rs_->GetXllCenter());
    ...
    EXPECT_FLOAT_EQ(7.07, rs_->GetMinimum());
    EXPECT_FLOAT_EQ(98.49, rs_->GetMaximum());
    EXPECT_FLOAT_EQ(10.23766667, rs_->GetAverage());
    EXPECT_FLOAT_EQ(11.52952953, rs_->GetStd());
    EXPECT_FLOAT_EQ(91.42, rs_->GetRange());
    ...
    // Set core file name
    string newcorename = corename + "_1D-nopos_incst-mask-nopos-ext";
    ...
    EXPECT_FLOAT_EQ(9.85f, rs_->GetValueByIndex(60, 1));
    EXPECT_FLOAT_EQ(-9999.f, rs_->GetValueByIndex(61, 1));
    EXPECT_FLOAT_EQ(9.64f, rs_->GetValueByIndex(16));
    EXPECT_FLOAT_EQ(-9999.f, rs_->GetValueByIndex(16, 0));
    EXPECT_FLOAT_EQ(-9999.f, rs_->GetValueByIndex(-1, 2));
    EXPECT_FLOAT_EQ(-9999.f, rs_->GetValueByIndex(541, 2));
}
#ifdef USE_GDAL
INSTANTIATE_TEST_CASE_P(SingleLayer, clsRasterDataTestNoPosIncstMaskNoPosExt,
