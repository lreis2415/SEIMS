...
public:
    clsRasterDataTestMultiPosIncstMaskPosNoExt() : rs_(nullptr), maskrs_(nullptr) {
    }

    virtual ~clsRasterDataTestMultiPosIncstMaskPosNoExt() { delete rs_; }

    void SetUp() OVERRIDE {
        // Read mask data with default parameters, i.e., calculate valid positions.
        maskrs_ = clsRasterData<int>::Init(GetParam()->mask_name, true);
        ...
        rs_ = clsRasterData<float, int>::Init(filenames, true, maskrs_, false);
...
TEST_P(clsRasterDataTestMultiPosIncstMaskPosNoExt, RasterIO) {
    ...
    EXPECT_EQ(64, rs_->GetDataLength()); // m_nCells
    EXPECT_EQ(64, rs_->GetCellNumber()); // m_nCells
    ...
    clsRasterData<float, int>* mongors = clsRasterData<float, int>::Init(gfs, gfsfilename.c_str(), true, maskrs_, false);
    // test mongors data
