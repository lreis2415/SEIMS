 ...
 *            clsRasterDataTestPosIncstMaskPosNoExt
 *
...
class clsRasterDataTestPosIncstMaskPosNoExt: public TestWithParam<InputRasterFiles *> {
public:
    clsRasterDataTestPosIncstMaskPosNoExt() : rs_(nullptr), maskrs_(nullptr) {
    }

    virtual ~clsRasterDataTestPosIncstMaskPosNoExt() { delete rs_; }
    ...
    void SetUp() OVERRIDE {
        ...
        rs_ = clsRasterData<float, int>::Init(GetParam()->raster_name, true, maskrs_, false);
...
TEST_P(clsRasterDataTestPosIncstMaskPosNoExt, RasterIO) {
    /// 1. Test members after constructing.
    EXPECT_EQ(60, rs_->GetDataLength()); // m_nCells
    ...
    EXPECT_FLOAT_EQ(9.75f, rs_->GetValueByIndex(16));
    EXPECT_FLOAT_EQ(9.95f, rs_->GetValueByIndex(18));
    ...
    EXPECT_FLOAT_EQ(8.77f, rs_->GetValue(1, 4));
    EXPECT_FLOAT_EQ(8.77f, rs_->GetValue(1, 4, 1));
    ...
    // Get position
    EXPECT_EQ(8, rs_->GetPosition(22.05f, 37.95f)); // row 1, col 2
    EXPECT_EQ(8, rs_->GetPosition(23.95f, 36.05f));
    ...
    // Set core file name
    string newcorename = corename + "_1D-pos_incst-mask-pos-noext";
...
#ifdef USE_GDAL
INSTANTIATE_TEST_CASE_P(SingleLayer, clsRasterDataTestPosIncstMaskPosNoExt,
