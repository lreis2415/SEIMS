...
void Init2WayPartition(ctrl_t *ctrl, graph_t *graph, real_t *ntpwgts,
    ...
    switch (ctrl->iptype) {
        ...
        default:gk_errexit(SIGERR, "Unknown initial partition type: %d\n", ctrl->iptype);
    }
...
void InitSeparator(ctrl_t *ctrl, graph_t *graph, idx_t niparts) {
    ...
    switch (ctrl->iptype) {
        ...
        case METIS_IPTYPE_NODE:GrowBisectionNode(ctrl, graph, ntpwgts, niparts);
            break;

        default:gk_errexit(SIGERR, "Unkown iptype of %"PRIDX"\n", ctrl->iptype);
    }
