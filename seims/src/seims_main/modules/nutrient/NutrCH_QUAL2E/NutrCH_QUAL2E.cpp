...
#pragma omp parallel for
        for (int i = 0; i < m_nCells; i++) {
            ...
            int irch = m_rchID[i];
            //if (m_nReaches == 1) {  // deprecated code, left for remained. lj
            //    subi = 1;
            //} else
...
void NutrCH_QUAL2E::RouteOut(const int i) {
    ...
    FLTPT wtrTotal = m_chStorage[i] + m_rteWtrOut[i]; // m^3
...
void NutrCH_QUAL2E::NutrientTransform(const int i) {
    FLTPT thbc1 = 1.083; ///temperature adjustment factor for local biological oxidation of NH3 to NO2
    FLTPT thbc2 = 1.047; ///temperature adjustment factor for local biological oxidation of NO2 to NO3
    FLTPT thbc3 = 1.04;  ///temperature adjustment factor for local hydrolysis of organic N to ammonia N
    FLTPT thbc4 = 1.047; ///temperature adjustment factor for local decay of organic P to dissolved P

    FLTPT thgra = 1.047; ///temperature adjustment factor for local algal growth rate
    FLTPT thrho = 1.047; ///temperature adjustment factor for local algal respiration rate

    FLTPT thm_rk1 = 1.047; ///temperature adjustment factor for local CBOD deoxygenation
    FLTPT thm_rk2 = 1.024; ///temperature adjustment factor for local oxygen reaeration rate
    FLTPT thm_rk3 = 1.024; ///temperature adjustment factor for loss of CBOD due to settling
    FLTPT thm_rk4 = 1.060; ///temperature adjustment factor for local sediment oxygen demand

    FLTPT thrs1 = 1.024; ///temperature adjustment factor for local algal settling rate
    FLTPT thrs2 = 1.074; ///temperature adjustment factor for local benthos source rate for dissolved phosphorus
    FLTPT thrs3 = 1.074; ///temperature adjustment factor for local benthos source rate for ammonia nitrogen
    FLTPT thrs4 = 1.024; ///temperature adjustment factor for local organic N settling rate
    FLTPT thrs5 = 1.024; ///temperature adjustment factor for local organic P settling rate
    // Currently, by junzhi
    ...
    FLTPT wtrTotal = m_rteWtrOut[i] + m_chStorage[i]; /// m3
    if (m_chWtrDepth[i] <= 0.01) {
        m_chWtrDepth[i] = 0.01;
    }
    if (wtrTotal <= 0.) {
        /// which means no flow out of current channel    || wtrOut <= 0.
        m_chAlgae[i] = 0.;
        m_chChlora[i] = 0.;
        m_chOrgN[i] = 0.;
        m_chNH4[i] = 0.;
        ...
        m_chCOD[i] = 0.;
        m_chSatDOx = 0.;
        return; // return and route out with 0.
    }
    ...
    //FLTPT coef = 0.;
    ///// deoxygenation rate
    //coef = CalExp(-1. * corTempc(m_rk1[i], thm_rk1, wtmp)*tday);
    //FLTPT tmp = coef * cbodcon;
    //// cbod rate loss due to setting
    //coef = CalExp(-1. * corTempc(m_rk3[i], thm_rk1, wtmp)*tday);
    //tmp *= coef;
    //dbod = tmp;
    ...
    FLTPT uu = 0.; // variable to hold intermediate calculation result
    FLTPT vv = 0.; // variable to hold intermediate calculation result
    ...
    //m_rk2[i] =1.;	// Why define this value?

    //FLTPT hh = corTempc(m_rk2[i], thm_rk2, wtmp);
    ...
    //FLTPT doxrch = m_chSatDOx;
    // cbod deoxygenation
    //coef = CalExp(-0.1f * ww);
    //doxrch *= coef;
    // benthic sediment oxidation
    //coef = 1. - corTempc(m_rk4[i], thm_rk4, wtmp) / 100.;
    //doxrch *= coef;
    // ammonia oxydation
    //coef = CalExp(-0.05f * yy);
    //doxrch *= coef;
    // nitrite oxydation
    //coef = CalExp(-0.05f * zz);
    //doxrch *= coef;
    // reaeration
    //uu = corTempc(m_rk2[i], thm_rk2, wtmp) / 100. * (m_chSatDOx - doxrch);
    //ddisox = doxrch + uu;
    //if (ddisox < 1.e-6f) ddisox = 0.;
