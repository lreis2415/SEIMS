...
bool IUH_NUTR_OL::CheckInputData(void) {
    ...
    if (m_nSubbasins <= 0) {
        throw ModelException(MID_IUH_NUTR_OL, "CheckInputData", "The subbasins number must be greater than 0.");
    }
    if (m_subbasinIDs.empty()) {
        throw ModelException(MID_IUH_NUTR_OL, "CheckInputData", "The subbasin IDs can not be EMPTY.");
    }
