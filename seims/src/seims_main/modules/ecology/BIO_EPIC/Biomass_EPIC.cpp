...
Biomass_EPIC::Biomass_EPIC() : m_nCells(-1), m_co2Conc(NODATA_VALUE), m_meanTemp(nullptr), m_minTemp(nullptr),
...
void Biomass_EPIC::SetValue(const char* key, const float value) {
    ...
    else if (StringMatch(sk, VAR_NUPDIS)) m_upTkDistN = value;
...
void Biomass_EPIC::Set1DData(const char* key, const int n, float* data) {
    ...
    else if (StringMatch(sk, VAR_SOL_ZMX)) m_soilMaxRootD = data;
    ...
    else if (StringMatch(sk, VAR_ALAIMIN)) m_minLaiDorm = data;
    else if (StringMatch(sk, VAR_BIO_E)) m_biomEnrgRatio = data;
    else if (StringMatch(sk, VAR_BIOEHI)) m_biomEnrgRatio2ndPt = data;
    else if (StringMatch(sk, VAR_BIOLEAF)) m_biomDropFr = data;
    else if (StringMatch(sk, VAR_BLAI)) m_maxLai = data;
    else if (StringMatch(sk, VAR_BMX_TREES)) m_maxBiomTree = data;
    else if (StringMatch(sk, VAR_BN1)) m_biomNFr1 = data;
    else if (StringMatch(sk, VAR_BN2)) m_biomNFr2 = data;
    else if (StringMatch(sk, VAR_BN3)) m_biomNFr3 = data;
    ...
    else if (StringMatch(sk, VAR_CHTMX)) m_maxCanHgt = data;
    else if (StringMatch(sk, VAR_CO2HI)) m_co2Conc2ndPt = data;
    else if (StringMatch(sk, VAR_DLAI)) m_dormPHUFr = data;
    else if (StringMatch(sk, VAR_EXT_COEF)) m_lightExtCoef = data;
    else if (StringMatch(sk, VAR_FRGRW1)) m_frGrow1stPt = data;
    else if (StringMatch(sk, VAR_FRGRW2)) m_frGrow2ndPt = data;
    else if (StringMatch(sk, VAR_HVSTI)) m_hvstIdx = data;
    else if (StringMatch(sk, VAR_LAIMX1)) m_frMaxLai1stPt = data;
    else if (StringMatch(sk, VAR_LAIMX2)) m_frMaxLai2ndPt = data;
    else if (StringMatch(sk, VAR_MAT_YRS)) m_matYrs = data;
    else if (StringMatch(sk, VAR_T_BASE)) m_pgTempBase = data;
    else if (StringMatch(sk, VAR_T_OPT)) m_pgOptTemp = data;
    else if (StringMatch(sk, VAR_WAVP)) m_wavp = data;
    else if (StringMatch(sk, VAR_EPCO)) m_epco = data;
    else if (StringMatch(sk, VAR_TREEYRS)) m_curYrMat = data;
...
void Biomass_EPIC::Get1DData(const char* key, int* n, float** data) {
    ...
    *n = m_nCells;
    ...
    else if (StringMatch(sk, VAR_LAST_SOILRD)) *data = m_stoSoilRootD;
    ...
    else if (StringMatch(sk, VAR_PLTPET_TOT)) *data = m_totPltPET;
