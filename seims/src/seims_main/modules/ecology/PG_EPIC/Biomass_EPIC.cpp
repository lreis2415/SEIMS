...
Biomass_EPIC::Biomass_EPIC() :
    ...
    ubw(10.), uobw(NODATA_VALUE), m_pltRootD(nullptr), m_wuse(nullptr),
    m_lai(nullptr),
...
void Biomass_EPIC::SetValue(const char* key, const FLTPT value) {
    string sk(key);
...
void Biomass_EPIC::Set1DData(const char* key, const int n, FLTPT* data) {
    string sk(key);
    //// climate
...
void Biomass_EPIC::Set1DData(const char* key, const int n, int* data) {
    string sk(key);
    //// climate
...
void Biomass_EPIC::Set2DData(const char* key, const int nrows, const int ncols, FLTPT** data) {
    string sk(key);
...
bool Biomass_EPIC::CheckInputData() {
    ...
    CHECK_NODATA(GetModuleName(), m_co2Conc);
    CHECK_NODATA(GetModuleName(), m_upTkDistN);
    CHECK_NODATA(GetModuleName(), m_upTkDistP);
    CHECK_NODATA(GetModuleName(), m_NFixCoef);
    CHECK_NODATA(GetModuleName(), m_NFixMax);
    /// DT_Raster1D
    CHECK_POINTER(GetModuleName(), m_annMeanTemp);
    CHECK_POINTER(GetModuleName(), m_minTemp);
    CHECK_POINTER(GetModuleName(), m_meanTemp);
    CHECK_POINTER(GetModuleName(), m_SR);
    CHECK_POINTER(GetModuleName(), m_dayLen);
    CHECK_POINTER(GetModuleName(), m_dayLenMin);
    CHECK_POINTER(GetModuleName(), m_dormHr);
    CHECK_POINTER(GetModuleName(), m_nSoilLyrs);
    CHECK_POINTER(GetModuleName(), m_soilMaxRootD);
    CHECK_POINTER(GetModuleName(), m_soilAlb);
    CHECK_POINTER(GetModuleName(), m_soilWtrStoPrfl);
    CHECK_POINTER(GetModuleName(), m_soilSumFC);
    CHECK_POINTER(GetModuleName(), m_soilSumSat);
    CHECK_POINTER(GetModuleName(), m_pet);
    CHECK_POINTER(GetModuleName(), m_vpd);
    CHECK_POINTER(GetModuleName(), m_maxPltET);
    CHECK_POINTER(GetModuleName(), m_soilET);
    CHECK_POINTER(GetModuleName(), m_rsdInitSoil);
    CHECK_POINTER(GetModuleName(), m_igro);
    CHECK_POINTER(GetModuleName(), m_landCoverCls);
    CHECK_POINTER(GetModuleName(), m_minLaiDorm);
    CHECK_POINTER(GetModuleName(), m_biomEnrgRatio);
    CHECK_POINTER(GetModuleName(), m_biomEnrgRatio2ndPt);
    CHECK_POINTER(GetModuleName(), m_biomDropFr);
    CHECK_POINTER(GetModuleName(), m_maxLai);
    CHECK_POINTER(GetModuleName(), m_maxBiomTree);
    CHECK_POINTER(GetModuleName(), m_biomNFr1);
    CHECK_POINTER(GetModuleName(), m_biomNFr2);
    CHECK_POINTER(GetModuleName(), m_biomNFr3);
    CHECK_POINTER(GetModuleName(), m_biomPFr1);
    CHECK_POINTER(GetModuleName(), m_biomPFr2);
    CHECK_POINTER(GetModuleName(), m_biomPFr3);
    CHECK_POINTER(GetModuleName(), m_maxCanHgt);
    CHECK_POINTER(GetModuleName(), m_co2Conc2ndPt);
    CHECK_POINTER(GetModuleName(), m_dormPHUFr);
    CHECK_POINTER(GetModuleName(), m_lightExtCoef);
    CHECK_POINTER(GetModuleName(), m_frGrow1stPt);
    CHECK_POINTER(GetModuleName(), m_frGrow2ndPt);
    CHECK_POINTER(GetModuleName(), m_hvstIdx);
    CHECK_POINTER(GetModuleName(), m_frMaxLai1stPt);
    CHECK_POINTER(GetModuleName(), m_frMaxLai2ndPt);
    CHECK_POINTER(GetModuleName(), m_matYrs);
    CHECK_POINTER(GetModuleName(), m_pgTempBase);
    CHECK_POINTER(GetModuleName(), m_pgOptTemp);
    CHECK_POINTER(GetModuleName(), m_wavp);
    CHECK_POINTER(GetModuleName(), m_epco);
    CHECK_POINTER(GetModuleName(), m_curYrMat);
    CHECK_POINTER(GetModuleName(), m_initLai);
    CHECK_POINTER(GetModuleName(), m_initBiom);
    CHECK_POINTER(GetModuleName(), m_phuPlt);
    CHECK_POINTER(GetModuleName(), m_canHgt);
    CHECK_POINTER(GetModuleName(), m_dormFlag);
    /// DT_Raster2D
    ...
    CHECK_POINTER(GetModuleName(), m_soilNO3);
    CHECK_POINTER(GetModuleName(), m_soilSolP);
    return true;
}

void Biomass_EPIC::InitialOutputs() {
    if (FloatEqual(uobw, NODATA_VALUE)) {
        ubw = 10.; /// the uptake distribution for water is hardwired, users are not allowed to modify
        uobw = 0.;
...
#pragma omp parallel for
    ...
    if (m_biomTrgt == nullptr) {
        Initialize1DArray(m_nCells, m_biomTrgt, 0.);
    }
    if (m_lai == nullptr) {
        if (m_initLai != nullptr) {
            Initialize1DArray(m_nCells, m_lai, m_initLai);
        } else {
            Initialize1DArray(m_nCells, m_lai, 0.);
        }
    }
    if (m_maxLaiYr == nullptr) {
        m_maxLaiYr = new(nothrow) FLTPT[m_nCells];
#pragma omp parallel for
        for (int i = 0; i < m_nCells; i++) {
            if (IsTree(m_landCoverCls[i])) {
    ...
    if (m_phuAccum == nullptr) {
        Initialize1DArray(m_nCells, m_phuAccum, 0.);
    }
    if (m_totActPltET == nullptr) {
        Initialize1DArray(m_nCells, m_totActPltET, 0.);
    }
    if (m_totPltPET == nullptr) {
        Initialize1DArray(m_nCells, m_totPltPET, 0.);
    }
    if (m_hvstIdxAdj == nullptr) {
        Initialize1DArray(m_nCells, m_hvstIdxAdj, 0.);
    }
    if (m_LaiMaxFr == nullptr) {
        Initialize1DArray(m_nCells, m_LaiMaxFr, 0.);
    }
    if (m_oLai == nullptr) {
        Initialize1DArray(m_nCells, m_oLai, 0.);
    }
    if (m_stoSoilRootD == nullptr) {
        Initialize1DArray(m_nCells, m_stoSoilRootD, 10.);
    }
    if (m_pltRootD == nullptr) {
        Initialize1DArray(m_nCells, m_pltRootD, 10.);
    }
    if (nullptr == m_wuse) {
        Initialize2DArray(m_nCells, m_maxSoilLyrs, m_wuse, 0.);
    }
    ...
    if (m_fixN == nullptr) {
        Initialize1DArray(m_nCells, m_fixN, 0.);
    }
    if (m_plantUpTkN == nullptr) {
        Initialize1DArray(m_nCells, m_plantUpTkN, 0.);
    }
    if (m_plantUpTkP == nullptr) {
        Initialize1DArray(m_nCells, m_plantUpTkP, 0.);
    }
    if (m_pltN == nullptr) {
        Initialize1DArray(m_nCells, m_pltN, 0.);
    }
    if (m_pltP == nullptr) {
        Initialize1DArray(m_nCells, m_pltP, 0.);
    }
    if (m_frPltN == nullptr) {
        Initialize1DArray(m_nCells, m_frPltN, 0.);
    }
    if (m_frPltP == nullptr) {
        Initialize1DArray(m_nCells, m_frPltP, 0.);
    }

    if (m_NO3Defic == nullptr) {
        Initialize1DArray(m_nCells, m_NO3Defic, 0.);
    }
    // initialize these stress factors according to sim_iniday.f of SWAT
    if (m_frStrsAe == nullptr) {
        Initialize1DArray(m_nCells, m_frStrsAe, 1.);
    }
    if (m_frStrsN == nullptr) {
        Initialize1DArray(m_nCells, m_frStrsN, 1.);
    }
    if (m_frStrsP == nullptr) {
        Initialize1DArray(m_nCells, m_frStrsP, 1.);
    }
    if (m_frStrsTmp == nullptr) {
        Initialize1DArray(m_nCells, m_frStrsTmp, 1.);
    }
    ...
    if (m_biomassDelta == nullptr) {
        Initialize1DArray(m_nCells, m_biomassDelta, 0.);
    }
...
void Biomass_EPIC::DistributePlantET(const int i) {
    ...
    for (int j = 0; j < CVT_INT(m_nSoilLyrs[i]); j++) m_wuse[i][j] = 0.;
    ...
    if (idc == CROP_IDC_WARM_SEASON_ANNUAL_LEGUME ||
        ...
        m_pltRootD[i] = 2.5 * m_phuAccum[i] * m_soilMaxRootD[i];
        if (m_pltRootD[i] > m_soilMaxRootD[i]) m_pltRootD[i] = m_soilMaxRootD[i];
        if (m_pltRootD[i] < 10.) m_pltRootD[i] = 10.; /// minimum root depth is 10mm
    } else {
        m_pltRootD[i] = m_soilMaxRootD[i];
    }
    m_stoSoilRootD[i] = m_pltRootD[i];
    if (m_maxPltET[i] <= 0.01) {
        m_frStrsWtr[i] = 1.;
        m_actPltET[i] = 0.;
        return;
    }
    ...
    if (m_soilWtrStoPrfl[i] >= m_soilSumFC[i]) {
        FLTPT satco = (m_soilWtrStoPrfl[i] - m_soilSumFC[i]) / (m_soilSumSat[i] - m_soilSumFC[i]);
        FLTPT pl_aerfac = 0.85;
        FLTPT scparm = 100. * (satco - pl_aerfac) / (1.0001 - pl_aerfac);
        if (scparm > 0.) {
            m_frStrsAe[i] = 1. - (scparm / (scparm + CalExp(2.9014 - 0.03867 * scparm)));
        } else {
            m_frStrsAe[i] = 1.;
        }
    }
    for (int j = 0; j < CVT_INT(m_nSoilLyrs[i]); j++) {
        ...
        if (m_pltRootD[i] <= 0.01) {
            sum = m_maxPltET[i] / uobw;
        } else {
            sum = m_maxPltET[i] * (1. - CalExp(-ubw * gx / m_pltRootD[i])) / uobw;
        }
        m_wuse[i][j] = sum - sump + 1. * m_epco[i];
        m_wuse[i][j] = sum - sump + (sump - xx) * m_epco[i];
        sump = sum;
        /// adjust uptake if sw is less than 25% of plant available water
        if (m_soilWtrSto[i][j] < m_soilFC[i][j] * 0.25) {
            reduc = CalExp(5. * (4. * m_soilWtrSto[i][j] / m_soilFC[i][j] - 1.));
        } else {
            reduc = 1.;
        }
        // reduc = 1.;  /// TODO, Is SWAT wrong here? by LJ
        m_wuse[i][j] *= reduc;
...
void Biomass_EPIC::CalTempStress(const int i) {
    ...
    if (tgx <= 0.f) {
        ...
        tgx = 2. * m_pgOptTemp[i] - m_pgTempBase[i] - m_meanTemp[i];
    }
    rto = (m_pgOptTemp[i] - m_pgTempBase[i]) / CalPow(tgx + UTIL_ZERO, 2.);
    if (rto <= 200. && tgx > 0.) {
        m_frStrsTmp[i] = CalExp(-0.1054 * rto);
    } else {
        m_frStrsTmp[i] = 0.;
    }
    if (m_minTemp[i] <= m_annMeanTemp[i] - 15.) {
        m_frStrsTmp[i] = 0.;
    }
}
...
void Biomass_EPIC::AdjustPlantGrowth(const int i) {
    ...
    if (m_phuAccum[i] <= 1.) {
        ...
        FLTPT activeRadiation = 0.;
        activeRadiation = 0.5 * m_SR[i] * (1. - CalExp(-m_lightExtCoef[i] * (m_lai[i] + 0.05)));
        ...
        if (FloatEqual(m_co2Conc2ndPt[i], 330.)) m_co2Conc2ndPt[i] = 660.;
        FLTPT m_RadUseEffiShpCoef1 = 0.;
        FLTPT m_RadUseEffiShpCoef2 = 0.;
        GetScurveShapeParameter(m_biomEnrgRatio[i] * 0.01, m_biomEnrgRatio2ndPt[i] * 0.01, m_co2Conc,
                                m_co2Conc2ndPt[i],
                                &m_RadUseEffiShpCoef1, &m_RadUseEffiShpCoef2);

        FLTPT beadj = 0.;
        if (m_co2Conc > 330.) {
            beadj = 100. * m_co2Conc / (m_co2Conc + CalExp(m_RadUseEffiShpCoef1 - m_co2Conc * m_RadUseEffiShpCoef2));
        } else {
        ...
        if (m_vpd[i] > 1.) {
            beadj -= m_wavp[i] * (m_vpd[i] - 1.);
            beadj = Max(beadj, 0.27 * m_biomEnrgRatio[i]);
        }
        m_biomassDelta[i] = Max(0., beadj * activeRadiation);
        ...
        FLTPT reg = Min(Min(m_frStrsWtr[i], m_frStrsTmp[i]), m_frStrsAe[i]);
        if (reg < 0.) reg = 0.;
        if (reg > 0.) {
        ...
        if (idc == CROP_IDC_TREES) {
            if (m_matYrs[i] > 0.) {
                FLTPT curYrMat = m_curYrMat[i] + m_yearIdx;
                rto = curYrMat / m_matYrs[i];
                m_biomass[i] = Min(m_biomass[i], rto * m_maxBiomTree[i] * 1000.); /// convert tons/ha -> kg/ha
            } else {
                rto = 1.;
            }
        }
        m_biomass[i] = Max(m_biomass[i], 0.);
        //if(i == 5) cout << m_biomass[i] << ", \n";
        /// 7. calculate fraction of total biomass that is in the roots
        FLTPT m_rootShootRatio1 = 0.4;
        FLTPT m_rootShootRatio2 = 0.2;
        ...
        FLTPT f = m_phuAccum[i] / (m_phuAccum[i] + CalExp(LAIShpCoef1 - LAIShpCoef2 * m_phuAccum[i]));
        FLTPT ff = f - m_LaiMaxFr[i];
        m_LaiMaxFr[i] = f;
        /// 8. calculate new canopy height
        if (idc == CROP_IDC_TREES) {
            m_canHgt[i] = rto * m_maxCanHgt[i];
        } else {
            m_canHgt[i] = m_maxCanHgt[i] * CalSqrt(f);
        }
        ...
        if (m_phuAccum[i] <= m_dormPHUFr[i]) {
            ...
            laiDelta = ff * laiMax * (1. - CalExp(5. * (m_lai[i] - laiMax))) * CalSqrt(reg);
            m_lai[i] += laiDelta;
            if (m_lai[i] > laiMax) m_lai[i] = laiMax;
            m_oLai[i] = m_lai[i];
            if (m_lai[i] > m_maxLaiYr[i]) m_maxLaiYr[i] = m_lai[i];
        } else {
            m_lai[i] = m_oLai[i] * (1. - m_phuAccum[i]) / (1. - m_dormPHUFr[i]);
        }
        if (m_lai[i] < m_minLaiDorm[i]) {
            m_lai[i] = m_minLaiDorm[i];
        }
        /// 10. calculate plant ET values
        if (m_phuAccum[i] > 0.5 && m_phuAccum[i] < m_dormPHUFr[i]) {
            m_totActPltET[i] += m_actPltET[i] + m_soilET[i];
            m_totPltPET[i] += m_pet[i];
        }
        m_hvstIdxAdj[i] = m_hvstIdx[i] * 100. * m_phuAccum[i] / (100. * m_phuAccum[i] +
            CalExp(11.1 - 10. * m_phuAccum[i]));
    } else {
        if (m_dormPHUFr[i] > 1.) {
            if (m_phuAccum[i] > m_dormPHUFr[i]) {
                m_lai[i] = m_oLai[i] * (1. - (m_phuAccum[i] - m_dormPHUFr[i]) / (1.2 - m_dormPHUFr[i]));
            }
        }
        if (m_lai[i] < 0.) m_lai[i] = 0.;
    }
}

void Biomass_EPIC::PlantNitrogenUptake(const int i) {
    FLTPT uobn = GetNormalization(m_upTkDistN);
    ...
    tno3 /= n_reduc;
    // float up_reduc = tno3 / (tno3 + CalExp(1.56f - 4.5f * tno3)); /// However, up_reduc is not used hereafter.
    ...
    FLTPT un2 = 0.; /// ideal (or optimal) plant nitrogen content (kg/ha)
    un2 = m_frPltN[i] * m_biomass[i];
    if (un2 < m_pltN[i]) un2 = m_pltN[i];
    m_NO3Defic[i] = un2 - m_pltN[i];
    m_NO3Defic[i] = Min(4. * m_biomNFr3[i] * m_biomassDelta[i], m_NO3Defic[i]);
    m_frStrsN[i] = 1.;
    int ir = 0;
    ...
    for (int l = 0; l < CVT_INT(m_nSoilLyrs[i]); l++) {
        ...
        FLTPT unmx = 0.;
        FLTPT uno3l = 0.; /// plant nitrogen demand (kg/ha)
        unmx = m_NO3Defic[i] * (1. - CalExp(-m_upTkDistN * gx / m_pltRootD[i])) / uobn;
...
void Biomass_EPIC::PlantNitrogenFixed(const int i) {
    ...
    FLTPT fxw = 0.;
    fxw = m_soilWtrStoPrfl[i] / (0.85 * m_soilSumFC[i]);
    //// 2. compute no3 factor
    FLTPT sumn = 0.; /// total amount of nitrate stored in soil profile (kg/ha)
    FLTPT fxn = 0.;
    ...
    FLTPT fxg = 0.;
    if (m_phuAccum[i] > 0.15 && m_phuAccum[i] <= 0.3) {
        fxg = 6.67 * m_phuAccum[i] - 1.;
    }
    if (m_phuAccum[i] > 0.3 && m_phuAccum[i] <= 0.55) {
        fxg = 1.;
    }
    if (m_phuAccum[i] > 0.55 && m_phuAccum[i] <= 0.75) {
        fxg = 3.75 - 5. * m_phuAccum[i];
    }
    FLTPT fxr = Min(1., Min(fxw, fxn)) * fxg;
    fxr = Max(0., fxr);
    if (m_NFixCoef <= 0.) m_NFixCoef = 0.5;
    if (m_NFixMax <= 0.) m_NFixMax = 20.;
    m_fixN[i] = Min(6., fxr * m_NO3Defic[i]);
    m_fixN[i] = m_NFixCoef * m_fixN[i] + (1. - m_NFixCoef) * uno3l;
    m_fixN[i] = Min(m_fixN[i], uno3l);
    m_fixN[i] = Min(m_NFixMax, m_fixN[i]);
}

void Biomass_EPIC::PlantPhosphorusUptake(const int i) {
    FLTPT uobp = GetNormalization(m_upTkDistP);
    ...
    FLTPT up2 = 0.;  /// optimal plant phosphorus content
    FLTPT uapd = 0.; /// plant demand of phosphorus
    FLTPT upmx = 0.; /// maximum amount of phosphorus that can be removed from the soil layer
    FLTPT uapl = 0.; /// amount of phosphorus removed from layer
    FLTPT gx = 0.;   /// lowest depth in layer from which phosphorus may be removed
    ...
    for (int l = 0; l < m_nSoilLyrs[i]; l++) {
        ...
        upmx = uapd * (1. - CalExp(-m_upTkDistP * gx / m_pltRootD[i])) / uobp;
        uapl = Min(upmx - m_plantUpTkP[i], m_soilSolP[i][l]);
        m_plantUpTkP[i] += uapl;
        m_soilSolP[i][l] -= uapl;
    }
    if (m_plantUpTkP[i] < 0.) m_plantUpTkP[i] = 0.;
...
#pragma omp parallel for
    for (int i = 0; i < m_nCells; i++) {
        ...
        FLTPT cej = -5.e-5;
        FLTPT eaj = CalExp(cej * (m_rsdCovSoil[i] + 0.1)); // eq. 1:1.2.16 in SWAT theory 2009
        if (m_snowAccum[i] < 0.5) {
            ...
            m_alb[i] = 0.8; // eq. 1:1.2.13
        }
        /// reWrite from plantmod.f of SWAT
        /// calculate residue on soil surface for current day
        m_rsdCovSoil[i] = Max(0.8 * m_biomass[i] + m_soilRsd[i][0], 0.);
        if (FloatEqual(m_igro[i], 1.)) {
            /// land cover growing
            DistributePlantET(i); /// swu.f
            if (FloatEqual(m_dormFlag[i], 0.)) {
...
void Biomass_EPIC::Get1DData(const char* key, int* n, FLTPT** data) {
    InitialOutputs();
    string sk(key);
    *n = m_nCells;
...
void Biomass_EPIC::Get1DData(const char* key, int* n, int** data) {
    InitialOutputs();
    string sk(key);
    *n = m_nCells;
    if (StringMatch(sk, VAR_DORMI[0])) *data = m_dormFlag;
    else if (StringMatch(sk, VAR_IGRO[0])) *data = m_igro;
    else {
        throw ModelException(GetModuleName(), "Get1DData",
                             "Integer Result " + sk + " does not exist.");
    }
}

void Biomass_EPIC::Get2DData(const char* key, int* nrows, int* ncols, FLTPT*** data) {
    InitialOutputs();
    string sk(key);
    *nrows = m_nCells;
    *ncols = m_maxSoilLyrs;
