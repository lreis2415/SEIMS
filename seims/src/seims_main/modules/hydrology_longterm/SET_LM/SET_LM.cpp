...
#pragma omp parallel for reduction(+: errCount)
    for (int i = 0; i < m_nCells; i++) {
        ...
        for (int j = 0; j < CVT_INT(m_nSoilLyrs[i]); j++) {
            ...
            if (isinf(m_soilWtrSto[i][j]) || isnan(m_soilWtrSto[i][j]) || m_soilWtrSto[i][j] < 0.f) {
    ...
    if (errCount > 0) {
        throw ModelException(M_SET_LM[0], "Execute", "Soil moisture can not less than zero!");
    }
    return 0;
}
...
void SET_LM::Get1DData(const char* key, int* nRows, float** data) {
    ...
    if (StringMatch(s, VAR_SOET[0])) *data = m_soilET;
        ...
        throw ModelException(M_SET_LM[0], "Get1DData", "Result " + s + " does not exist.");
    }
    *nRows = m_nCells;
}
...
void SET_LM::SetValue(const char* key, const float value) {
    ...
    if (StringMatch(s, VAR_T_SOIL[0])) m_soilFrozenTemp = value;
        ...
        throw ModelException(M_SET_LM[0], "SetValue", "Parameter " + s + " does not exist.");
    }
}
...
void SET_LM::Set1DData(const char* key, const int nrows, float* data) {
    ...
    if (StringMatch(s, VAR_SOILLAYERS[0])) m_nSoilLyrs = data;
    else if (StringMatch(s, VAR_INET[0])) m_IntcpET = data;
        ...
        throw ModelException(M_SET_LM[0], "Set1DData", "Parameter " + s + " does not exist.");
    }
}
...
void SET_LM::Set2DData(const char* key, const int nrows, const int ncols, float** data) {
    ...
    if (StringMatch(sk, VAR_SOL_AWC[0])) m_soilFC = data;
        ...
        throw ModelException(M_SET_LM[0], "Set2DData", "Parameter " + sk + " does not exist.");
    }
}
