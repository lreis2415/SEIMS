...
extern "C" SEIMS_MODULE_API const char* MetadataInformation() {
    ...
    mdi.SetDescription(M_GREL_LINEAR[1]);
    mdi.SetEmail("");
    mdi.SetID(M_GREL_LINEAR[0]);
    mdi.SetName(M_GREL_LINEAR[0]);
    mdi.SetVersion("0.1");
    mdi.SetWebsite("");
    mdi.SetHelpfile("GlacierReleaseLinear.chm");
    mdi.SetAbstractionTypeToConceptual();

    mdi.AddInput(VAR_GLAC_MELT[0], UNIT_DEPTH_MM, VAR_GLAC_MELT[1], Source_Module, DT_Raster1D);
    mdi.AddParameter(VAR_GLAC_STOR_COEF[0], UNIT_NON_DIM, VAR_GLAC_STOR_COEF[1], Source_ParameterDB, DT_Single);
