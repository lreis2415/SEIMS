...
void IO_TEST::Set1DData(const char* key, const int n, int* data) {
    if (!CheckInputSize("IO_TEST", key, n, m_nCells)) return;
    string sk(key);
    if (StringMatch(sk, VAR_SOILLAYERS[0])) m_nSoilLyrs = data;
    else {
        throw ModelException("IO_TEST", "Set1DData", "Integer Parameter " + string(key) + " is not exist");
    }
}

void IO_TEST::Set1DData(const char* key, const int n, FLTPT* data) {
    if (!CheckInputSize("IO_TEST", key, n, m_nCells)) return;
    string sk(key);
    if (StringMatch(sk, VAR_CN2[0])) m_raster1D = data;
    else {
        throw ModelException("IO_TEST", "Set1DData", "Float Parameter " + string(key) + " is not exist");
    }
}

void IO_TEST::Set2DData(const char* key, const int n, const int col, FLTPT** data) {
    string sk(key);
...
int IO_TEST::Execute() {
    ...
    if (nullptr == m_output1Draster) Initialize1DArray(m_nCells, m_output1Draster, 0.);
...
#pragma omp parallel for
    for (int i = 0; i < m_nCells; i++) {
        m_output1Draster[i] = m_raster1D[i] * 0.5;
        for (int j = 0; j < m_nSoilLyrs[i]; j++) {
            m_output2Draster[i][j] = m_raster2D[i][j] + 2.;
        }
    }
...
void IO_TEST::Get1DData(const char* key, int* n, FLTPT** data) {
    string sk(key);
    if (StringMatch(sk, "CN2_M")) {
        *data = m_output1Draster;
        *n = m_nCells;
    }
}

void IO_TEST::Get2DData(const char* key, int* n, int* col, FLTPT*** data) {
    string sk(key);
    if (StringMatch(sk, "K_M")) {
        *data = m_output2Draster;
        *n = m_nCells;
        *col = m_maxSoilLyrs;
    }
}
