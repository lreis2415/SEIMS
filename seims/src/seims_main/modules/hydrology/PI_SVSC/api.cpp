...
extern "C" SEIMS_MODULE_API const char *MetadataInformation() {
    ...
    mdi.SetAuthor("Alex Storey, Junzhi Liu, Liangjun Zhu");
    mdi.SetClass(MCLS_INTERC[0], MCLS_INTERC[1]);
    mdi.SetDescription(M_PI_SVSC[1]);
    mdi.SetEmail(SEIMS_EMAIL);
    mdi.SetID(M_PI_SVSC[0]);
    mdi.SetName(M_PI_SVSC[0]);
    mdi.SetVersion("1.0");
...
#ifndef STORM_MODE
    mdi.AddInput(VAR_PET[0], UNIT_DEPTH_MM, VAR_PET[1], Source_Module, DT_Raster1D,TF_None,TI_Storm); /// PET
#else
    ...
    mdi.AddOutput(VAR_INET[0], UNIT_DEPTH_MM, VAR_INET[1], DT_Raster1D, TF_None, TI_Storm);
