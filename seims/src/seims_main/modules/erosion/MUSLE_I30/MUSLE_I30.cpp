...
bool MUSLE_I30::CheckInputData(void) {
    ...
    if (m_cellSize <= 0) {
        throw ModelException("MUSLE_I30", "CheckInputData",
                             "The dimension of the input data can not be less than zero.");
    }
    if (m_cellWidth <= 0) {
        throw ModelException("MUSLE_I30", "CheckInputData", "The cell width can not be less than zero.");
    }
    ...
    if (this->m_flowacc == NULL) {
        throw ModelException("MUSLE_I30", "CheckInputData", "The flow accumulation can not be NULL.");
    }
    if (this->m_slope == NULL) throw ModelException("MUSLE_I30", "CheckInputData", "The slope can not be NULL.");
    if (this->m_snowAccumulation == NULL) {
        throw ModelException("MUSLE_I30", "CheckInputData", "The snow accumulation can not be NULL.");
    }
    if (this->m_surfaceRunoff == NULL) {
        throw ModelException("MUSLE_I30", "CheckInputData", "The surface runoff can not be NULL.");
    }
    ...
    if (this->m_t_concentration == NULL) {
        throw ModelException("MUSLE_I30", "CheckInputData", "The time of concentration can not be NULL.");
    }
    ...
    if (m_alpha_month == NULL) {
        ...
        for (int i = 0; i < 12; i++) {
            if (i == 0) { m_alpha_month[i] = this->m_p_stat[11][0] + this->m_p_stat[0][0] + this->m_p_stat[1][0]; }
                ...
                m_alpha_month[i] = this->m_p_stat[10][0] + this->m_p_stat[11][0] + this->m_p_stat[0][0];
            }
...
void MUSLE_I30::SetValue(const char *key, float data) {
    ...
    if (StringMatch(sk, "cellwidth")) { this->m_cellWidth = int(data); }
        ...
        throw ModelException("MUSLE_I30", "SetValue", "Parameter " + sk
            +
...
void MUSLE_I30::Set1DData(const char *key, int n, float *data) {
    ...
    if (StringMatch(s, "USLE_C")) { this->m_usle_c = data; }
        ...
        throw ModelException("MUSLE_I30", "SetValue", "Parameter " + s +
            " does not exist in MUSLE_I30 method. Please contact the module developer.");
    }
...
void MUSLE_I30::Get1DData(const char *key, int *n, float **data) {
    ...
    if (StringMatch(sk, "SOER")) {
        *data = this->m_sedimentYield;
        ...
        throw ModelException("MUSLE_I30", "getResult", "Result " + sk +
            " does not exist in MUSLE_I30 method. Please contact the module developer.");
    }
