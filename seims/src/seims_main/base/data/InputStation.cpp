...
InputStation::InputStation(MongoClient* conn, const time_t dtHillslope, const time_t dtChannel, string dataValueDirectory) :
    m_conn(conn), m_dtCh(dtChannel), m_dtHs(dtHillslope), m_dataValueDirectory(dataValueDirectory){
}
...
void InputStation::ReadSitesData(const string& hydroDBName, const string& sitesList, const string& siteType,
                                 const time_t startDate, const time_t endDate, const bool stormMode /* = false */, bool fileDB) {
    ...
    if (stormMode) {
        ...
        LOG(DEBUG) << "Use FileDB for SitesData";
        m_measurement[siteType] = new RegularMeasurement(m_dataValueDirectory,sitesList, siteTypeU,
                                                         startDate, endDate, m_dtHs);
    } else {
        LOG(DEBUG) << "Query MongoDB for SitesData";
