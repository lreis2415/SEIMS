...
DataCenter::DataCenter(InputArgs* input_args, ModuleFactory* factory, int subBasinID /* = 0 */) :
    m_modelName(input_args->m_model_name), m_modelPath(input_args->m_model_path),
    m_lyrMethod(input_args->m_layer_mtd), m_subbasinID(subBasinID),
    m_scenarioID(input_args->m_scenario_id), m_calibrationID(input_args->m_calibration_id),
    m_threadNum(input_args->m_thread_num),
    m_useScenario(false), m_outputScene(DB_TAB_OUT_SPATIAL),
    m_outputPath(""),
    m_modelMode(""), m_nSubbasins(-1), m_outletID(-1), m_factory(factory),
        ...
        if (id.find(MID_ITP) != string::npos) {
            ...
            for (size_t j = 0; j < parameters.size(); j++) {
                ...
                if (StringMatch(param->Name, Tag_VerticalInterpolation)) {
                    verticalInterpolation = param->Value > 0.f;
                    break;
                }
            }
        }
...
void DataCenter::UpdateInput(vector<SimulationModule *>& modules, time_t t) {
    ...
    for (size_t i = 0; i < n; i++) {
        string id = moduleIDs[i];
        SimulationModule* pModule = modules[i];
...
void DataCenter::updateParametersByScenario(int subbsnID) {
    ...
    for (auto iter = bmpFactories.begin(); iter != bmpFactories.end(); ++iter) {
        ...
        BMPArealStructFactory* tmpBMPArealStructFactory = dynamic_cast<BMPArealStructFactory *>(iter->second);
        map<int, BMPArealStruct *> arealbmps = tmpBMPArealStructFactory->getBMPsSettings();
        float* mgtunits = tmpBMPArealStructFactory->getRasterData();
        vector<int> selIDs = tmpBMPArealStructFactory->getUnitIDs();
