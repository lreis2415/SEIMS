...
void GridLayering::GetReverseDirMatrix() {
    ...
    for (int valid_idx = 0; valid_idx < n_valid_cells_; valid_idx++) {
        ...
        int stream_dst = handle_stream ? stream_matrix_[valid_idx] : -1;
        ...
        for (vector<int>::iterator it = flow_dirs.begin(); it != flow_dirs.end(); ++it) {
            ...
            int stream_src = handle_stream ? stream_matrix_[src_idx] : -1;
...
//#pragma omp parallel for
    for (int index = 0; index < n_valid_cells_; index++) {
        ...
        int stream_src = handle_stream ? stream_matrix_[index]: -1;
        ...
        for (vector<int>::iterator it = flow_dirs.begin(); it != flow_dirs.end(); ++it) {
            ...
            int stream_dst = handle_stream ? stream_matrix_[dst_idx] : -1;
            if (handle_stream && stream_src > 0 && stream_dst == stream_src) {
                continue;
            }
            flow_out_num_[index]++;
...
#pragma omp parallel for
    ...
    int layer_idx = 0;
    int index = 1;
    for (auto it = layer_tmp.rbegin();
         it != layer_tmp.rend(); ++it) {
...
bool GridLayering::GridLayeringEvenly() {
    ...
    if (force_outlet_ && !stream_file_.empty()) {
        fix_last_layer = false;
    }

    if (auto_fix_suffix) {
        fixed_suffix = AutoDetectFixedSuffix_Heuristic(N, K, fix_last_layer, Buckets, Count, low, &low_star);
    ...
    for (vuint64_t iter = 0; iter < max_iters; iter++) {
        ...
        if (!progress) {
            ++stall_rounds;
            if (stall_rounds >= 5) {
                int R_dyn = AutoDetectFixedSuffix_Heuristic(N, K, fix_last_layer, Buckets, Count, low, &low_star);
    ...
    for (int u = 0; u < n_valid_cells_; u++) {
        ...
        if (layers_evenly_) layers_evenly_[u] = CVT_FLT(lu + 1); // 1-based
    }
...
bool GridLayering::OutputGridLayering(const string &name, const int datalength,
    ...
    IntRaster(mask_, layer_grid, n_valid_cells_).OutputFileByGdal(outpath);
