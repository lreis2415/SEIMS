...
*               16-Sep-2025  Separate layering outputs: ROUTING_LAYERS_HILLSLOPE and _CHANNEL
*               22-Sep-2025  Use DEM data to make sure all cells can have downslope cells within the mask (for Dinf)
*---------------------------------------------------------------------*/
...
void Usage(const string& error_msg = "") {
    ...
    cout << " Usage: grid_layering -alg <flow_direction_algorithm> -outdir <output_dir> "
            "[-mask <mask_raster>] [-stream <stream_shp>] [-dem <dem_raster>] "
            "[-decimals <decimals>] "
...
int main(int argc, char** argv) {
    ...
    string dem_file;  // Filled DEM raster file
    ...
    while (argc > i) {
        ...
        } else if (StringMatch(argv[i], "-dem")) {
            i++;
            if (argc > i) {
                dem_file = argv[i];
                i++;
            } else {
                Usage("No argument followed '-dem'!");
            }
