...
*               01-Sep-2025  Rewrite evenly layering method
*               04-Sep-2025  Stream can be regarded as "outlets", therefore only layering hillslope cells
*---------------------------------------------------------------------*/
...
void Usage(const string& error_msg = "") {
    ...
    cout << " Usage: grid_layering -alg <flow_direction_algorithm> -outdir <output_dir> "
            "[-mask <mask_raster>] [-stream <stream_shp>] [-force_outlet <force_outlet>] "
            "[-force_inbasin <force_inbasin>] [-decimals <decimals>] "
    ...
    cout << "Ex.4. grid_layering -alg mfdmd -outdir d:/tmp -mask d:/test/mask.tif "
            "-stream d:/test/stream.shp -force_outlet 1 -file d:/test/fd_rasters mfdmd.tif,mfdfraction.tif 0"
            << endl << endl;
    exit(1);
}
...
int main(int argc, char** argv) {
    ...
    bool force_inbasin = true; // Force all cells flow inside the watershed
    bool from_file = false; // Use file for input flow direction data
    string mask_file; // Mask raster file, if from_file is true and mask_file is not specified, use input file instead
    string stream_file; // Shapefile generated by TauDEM, where 'LINKNO' is the field of stream value
    string input_dir; // Directory of input flow direction data
    string core_names; // Core filenames of flow direction data, e.g., dirCodeMFDmd.tif,fractionsMFDmd.tif
    ...
    int n_subbasins = -1; // 0 means the entire watershed, N (>1) means multiple subbasins, 1,2,3,...,N
    int decimals = 4; // round to N decimal places for flow fractions
    ...
    while (argc > i) {
        if (StringMatch(argv[i], "-alg")) {
            ...
            if (argc > i) {
                out_dir = argv[i];
                // if out directory doesn't exist, then create it
        ...
        } else if (StringMatch(argv[i], "-decimals")) {
            i++;
            if (argc > i) {
                decimals = CVT_INT(ToInt(argv[i])) > 0 ? CVT_INT(ToInt(argv[i])) : 4;
                i++;
            } else {
                Usage("No argument followed '-decimals'!");
            }
    ...
    int subbasin_start_id = 1; // Only used when from_mongo is true
    if (n_subbasins == 0) {
        subbasin_start_id = 0;
    }
    if (from_file) {
        if (fdtype == FD_D8) {
            ...
            GridLayeringD8* grid_lyr_d8 = new GridLayeringD8(n_subbasins, out_dir, in_file.c_str(),
            ...
            string ffrac_file = input_dir + SEP + in_files[1];
            if (mask_file.empty()) mask_file = cfdir_file;
            GridLayeringMFDmd* grid_lyr_mfdmd = new GridLayeringMFDmd(n_subbasins, out_dir,
                                                                      cfdir_file.c_str(),
                                                                      ffrac_file.c_str(),
                                                                      mask_file.c_str(),
                                                                      stream_file.c_str(),
        ...
        for (int idx = subbasin_start_id; idx <= n_subbasins; idx++) {
            if (fdtype == FD_D8) {
                GridLayeringD8* grid_lyr_d8 = new GridLayeringD8(idx, gfs, out_dir,
                                                                 stream_file.c_str(), force_outlet);
                grid_lyr_d8->Execute();
                delete grid_lyr_d8;
            } else if (fdtype == FD_Dinf) {
