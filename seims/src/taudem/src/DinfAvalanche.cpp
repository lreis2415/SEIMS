...
int avalancherunoutgrd(char *angfile, char *felfile, char *assfile, char *rzfile, char *dmfile, float thresh,
    ...
    MPI_Init(NULL, NULL);
    {
        ...
        for (int jj = 0; jj < totalX; jj++) { xcoord[jj] = bndbox[0] + (jj * ang.getdlon()); }
            ...
            while (!que.empty()) {
                ...
                if (!felData->isNodata(i, j)) {
                    if (!assData->isNodata(i, j) && assData->getData(i, j, tempShort) > 0) {
                        ...
                        im->setData(i,
                                    j,
                                    (int32_t) ig);  //  record global row and column so that lookup across partitions works
                        jm->setData(i, j, (int32_t) jg);
                        zm->setData(i,
                                    j,
                                    felData->getData(i,
                                                     j,
                                                     tempFloat));  //  Record max elevation in partition as global lookup later does not work across partitions
                            ...
                            if (p > 0.0 && p >= thresh) {
                                ...
                                if (rzz >= alpha) {
                                    ...
                                    if (path == 1) {
                                        ...
                                        flowData->localToGlobal((int) i, (int) j, ig, jg);
                                        float dxx = xcoord[ig]
                                            - xcoord[imm];//was jmm; imm and jmm are swiched because in the serial code they are used as j,i
