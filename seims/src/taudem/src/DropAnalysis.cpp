...
int dropan(char *areafile,
    ...
    MPI_Init(NULL, NULL);
    {
        ...
        if (rank == 0) {
            float timeestimate = (2e-7 * ssaTotalX * ssaTotalY * nthresh / pow((double) size, 0.65)) / 60
...
//					i=i;
                    ...
                    if (source) {
                        ...
                        bool
                            newstream = true;  // Flag to indicate whether the junction results in a new Strahler stream
                        for (k = 1; k <= 8; ++k) {
