...
bool sendLink(int32_t Id, int dest) {
    ...
    // MPI_Type_extent(MPI_LONG, &extent);  // deprecated in MPI-2 standard, replaced by ljzhu
    MPI_Type_get_extent(MPI_LONG, &lb, &extent);
    offsets[1] = 2 * extent;
    oldtypes[1] = MPI_FLOAT;
    blockcounts[1] = 3;
    //create define it as an MPI data type and commit it.
    // MPI_Type_struct(2, blockcounts, offsets, oldtypes, &PointType); // deprecated, replaced by ljzhu
...
bool recvLink(int src) {
    ...
    blockcounts[1] = 3;
    //create define it as an MPI data type and commit it.
    // MPI_Type_struct(2, blockcounts, offsets, oldtypes, &PointType); // deprecated in MPI-2 standard
    MPI_Type_create_struct(2, blockcounts, offsets, oldtypes, &PointType);
    MPI_Type_commit(&PointType);
    int flag;
    //MPI_Request req;
...
void appendPoint(int32_t Id, point *addPoint) {
    ...
    if (myLink == NULL) {
        MPI_Abort(MCW, 8);
    }//link not found cannot proccess
    //point* points;
        ...
        if (rank != 0) {
            for (i = 0; i < nx; i++) {//count links to send down
                if (!contribs->isNodata(i, -1) && contribs->getData(i, -1, tempShort)
                    < 0) {//i am on the boarder and I am negitive, check who points to me and send there id.
                ...
                if (!contribs->isNodata(i, ny) && contribs->getData(i, ny, tempShort)
                    ...
                    if (i > 0 && flowDir->getData((long) i - 1, (long) ny - 1, tempShort) == 8
                        ...
                        if (sendLink(idGrid->getData(i - 1, ny - 1, tempLong),
                                     rank + 1)) {//check to see if sent properly
                            linksSent++;
                        } else {//didn't send right.
                            MPI_Abort(MCW, 2);
                        }
                        ...
                        && idGrid->getData(i + 1, ny - 1, tempLong) >= 0) {
                        if (sendLink(idGrid->getData(i + 1, ny - 1, tempLong),
                                     rank + 1)) {//check to see if sent properl
                            linksSent++;
                        } else {//didn't send right.
                            MPI_Abort(MCW, 2);
                        }
