...
int sindexcombined(char *slopefile,
                   char *scaterrainfile,
                   char *scarminroadfile,
                   char *scarmaxroadfile,
                   char *tergridfile,
                   char *terparfile,
                   char *satfile,
                   char *sincombinedfile,
                   double Rminter,
                   double Rmaxter,
                   double *par) {
    MPI_Init(NULL, NULL);
    {
        ...
        if (!slp.compareTiff(sca)) { //Unhappy error message and error return
            if (rank == 0) {
                fprintf(stderr,
                        "Slope (%s) and Contributing area (%s) files have different dimensions",
                        slopefile,
                        scaterrainfile);
            }
            return 1;
        ...
        tiffIO *sca_min = NULL;
        if (scarminroadfile != NULL && scarminroadfile[0] == '\0') {
            sca_min = new tiffIO(scarminroadfile, FLOAT_TYPE);
            if (!slp.compareTiff(*sca_min)) { //Unhappy error message and error return
                if (rank == 0) {
                    fprintf(stderr,
                            "Slope (%s) and road min contributing area (%s) files have different dimensions",
                            slopefile,
                            scarminroadfile);
                }
                return 1;
        ...
        tiffIO *sca_max = NULL;
        if (scarmaxroadfile != NULL && scarmaxroadfile[0] == '\0') {
            sca_max = new tiffIO(scarmaxroadfile, FLOAT_TYPE);
            if (!slp.compareTiff(*sca_max)) { //Unhappy error message and error return
                if (rank == 0) {
                    fprintf(stderr,
                            "Slope (%s) and road max contributing area (%s) files have different dimensions",
                            slopefile,
                            scarmaxroadfile);
                }
                return 1;
...
double sindexcell(double s, double a, double c1, double c2,
    ...
    if (x1 > x2) {  /*  Error these are wrong way round - switch them  */
        ...
        if (fs1 >= 1) /*  Always stable */
        {
            ...
            if (w1 == 1) /* region 1 */
            {
                ...
                if (w2 == 1) /* region 2 */
                {
                    ...
                    y1 = cs / ss * (1 - r);
                    y2 = cs / ss * (1 - x1 * as * r);
                    cdf1 = (x2 * as - 1) / (x2 * as - x1 * as)
                        * f2s(c1 / ss, c2 / ss, cs * (1 - r) / ss * t1, cs * (1 - r) / ss * t2, 1);
...
double f3s(double x1, double x2, double y1, double y2, double b1,
    ...
    if (x2 < x1 || y2 < y1 || b2 < b1) {
        ...
        if (y1 == y2 || b1 == b2) {  /* degenerate on y or b - reverts to additive convolution */
...
double fai(double y1, double y2, double b1, double b2, double a) {
    ...
    if (y1 * b2 > y2 * b1) {                    /* Invoke symmetry and switch so that y1*b2 < y2*b1 */
        t = y1;
        y1 = b1;
        b1 = t;
        t = y2;
