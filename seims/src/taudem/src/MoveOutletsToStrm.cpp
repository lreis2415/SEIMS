...
int outletstosrc(char *pfile,
                 char *srcfile,
                 char *outletsdatasrc,
                 char *outletslayer,
                 int uselyrname,
                 int lyrno,
                 char *outletmoveddatasrc,
                 char *outletmovedlayer,
                 int maxdist) {
    ...
    MPI_Init(NULL, NULL);
    {
            ...
            if (hDSsh != NULL) {
                ...
                if (hDSshmoved != NULL) {
                    ...
                    if (strlen(outletmovedlayer) == 0) {
                        ...
                        layernameshmoved =
                            getLayername(outletmoveddatasrc); // get layer name which is file name without extension
        ...
        if (nxy == 0) {  // Attempt to exit gracefully on all processors
            if (rank == 0) {
        ...
        while (!done) {
            for (i = 0; i < nxy; i++) {
                ...
                if (flowData->isInPartition(tx, ty)) {
                    part_has[i] = rank;  // grab control if in partition
                ...
                if (flowData->isInPartition(tx, ty) && dist_moved[i] >= 0) {
                    ...
                    if (srcData->isNodata(tx, ty) || td
                        < itresh) {  //  If not on stream.  The rule is any value >= itresh is a stream, less or no data is not stream
                        //move the outlet
                        ...
                        if (dirn >= 1 && dirn <= 8 && dist_moved[i] < maxdist) {
                            ...
                            if (nextx >= 0 && nexty >= 0 && nextx < pTotalX
                                && nexty < pTotalY) {  // If is within global domain
                                outletsX[i] = nextx;
                                outletsY[i] = nexty;
                                //ismoved[i]=1;
                                dist_moved[i]++;
            ...
            for (i = 0; i < nxy; i++) {
                ...
                flowData->globalToLocal(toutletsX[i],
                                        toutletsY[i],
                                        txn,
                                        tyn);  //DGT with single partition this is being passed uninitialized information
                flowData->globalToLocal(outletsX[i], outletsY[i], tx, ty);
                if (flowData->isInPartition(txn, tyn)
                    && !(part_has[i] == rank)) {  // if an outlet not in partition is now in partition - grab it
        ...
        if (!rank) {
            for (i = 0; i < nxy; i++) {
                ...
                if (dist_moved[i]
                    <= 0) {  // DGT changed condition to dist_moved <=0 because original values kept whenever not moved for whatever reason
                    xnode[i] = origxnode[i];
                    ynode[i] = origynode[i];
                }
            }
        ...
        if (rank == 0) {
            ...
            while ((hFeaturesh = OGR_L_GetNextFeature(hLayersh)) != NULL) {
                ...
                if (hDSshmoved != NULL) {
                    hFeatureshmoved =
                        OGR_F_Create(OGR_L_GetLayerDefn(hLayershmoved)); // create new feature with null fields and no geometry
                    //
