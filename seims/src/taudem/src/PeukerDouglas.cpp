...
//Create empty partition to store new information
        tdpartition *selev;
        selev = CreateNewPartition(FLOAT_TYPE,
                                   totalX,
                                   totalY,
                                   dxA,
                                   dyA,
                                   felev.getNodata());//selev is to smooth the elevation
        ...
        for (y = 0; y < elevny; y++) {
            for (x = 0; x < elevnx; x++) {
                ...
                if (rank == 0 && y == 0 || rank == (size - 1) && y == (elevny - 1)) {
                    ...
                    selev->setData(x, y, elev->getData(x, y, floatTemp1));
                    ss->setData(x, y, (short) 0);
                } else {                        //  THIS is for all the remainder of cells

                    ss->setData((long) x,
                                (long) y,
                                (short) 1);                                // Initializing to 1 for all non edge grid cells
