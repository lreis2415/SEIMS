...
from pygeoc.utils.const import DEFAULT_NODATA, CCW_DCOL, CCW_DROW, D8DIR_TD_INFLOW_VALUES, D8DIR_TD_DELTA
...
def delineate_hillslopes_downstream_method(stream_raster, flow_dir_raster, hillslope_out, stream_value_method = 0):
    ...
    :return hillslope_out: If the input stream are sequenced, the output hillslope will be numbered:
                              - Header hillslope: MaxStreamID * 3 + 1
                              - Right hillslope: MaxStreamID * 3 + 2
                              - Left hillslope: MaxStreamID * 3 + 3
                           If not, the output hillslope will be numbered in imprevision order.
    """
                ...
                if inflownum == 0:
                    ...
                    while flag:
                        ...
                        if stream_data[tmprow][tmpcol] <= 0:
                            ...
                            if HillslopeMtxTmp[tmprow][tmpcol] > 0:  # run into a larger stream, end the downstream search
                                break
    ...
    for row in range(nrows):
        for col in range(ncols):
            ...
            if inflownum == 0:  # it is a one-order stream head
                HillslopeMtx[row][col] = currentID * currentMaxID + 1
                headstreamCoors.append((row, col))
                continue
            elif inflownum >= 2:  # it is a confluence, assign hillslope code to the non-stream inflow cells
                non_stream_inflow_count = 0
                for (tmprow, tmpcol) in inflowcoors:
                    if stream_data[tmprow][tmpcol] <= 0:
                        HillslopeMtx[tmprow][tmpcol] = stream_data[row][col] * currentMaxID + 1
            ...
            while not flag:
                ...
                if tmpstream <= 0:
                    ...
                    if int(flowd8_data[tmprow][tmpcol]) == D8DIR_TD_INFLOW_VALUES[d - 1]:
                        HillslopeMtx[tmprow][tmpcol] = currentID * currentMaxID + 2
            ...
            while not flag:
                ...
                if tmpstream <= 0:
                    ...
                    if int(flowd8_data[tmprow][tmpcol]) == D8DIR_TD_INFLOW_VALUES[d - 1] and \
                        ...
                        HillslopeMtx[tmprow][tmpcol] = currentID * currentMaxID + 3
                k += 1
                if k == 7:
                    flag = True
            ...
            for col in range(ncols):
                if HillslopeMtx[row][col] == stream_data[row][col] > 0:
                    if stream_value_method == 1:  # right hillslope
                        HillslopeMtx[row][col] = stream_data[row][col] * 3 + 2
                    elif stream_value_method == 2:  # left hillslope
                        HillslopeMtx[row][col] = stream_data[row][col] * 3 + 3
                    else:
