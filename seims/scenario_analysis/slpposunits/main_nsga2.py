...
def main(cfg):
    ...
    scoop_log('BMPs configure method: %s' % ('rule-based' if rule_cfg else 'random-based'))
    ...
    for gen in range(1, gen_num + 1):
        ...
        # scoop_log('Offspring size: %d' % len(offspring))
        ...
        # scoop_log('Evaluate pop size: %d' % invalid_ind_size)
        try:
            from scoop import futures
