...
class UnifiedOptimizerV2:
    ...
    def create_scenario(self):
        ...
        try:
            ...
            if self.cfg.use_surrogate:
                ...
                sa_cfg.scenario_db = self.cfg.scenario_db  # needed by Scenario.__init__ fast-path
            ...
            sa_cfg.preference_fusion_strategy = getattr(
                self.cfg, 'preference_fusion_strategy', 'merge_preferences'
            )
            ...
            if hasattr(self.cfg, 'enable_async'):
                ...
                # --resume 模式：注入 CONTINUE JSON 路径供 NSGA2 main 读取
                sa_cfg.async_continue_file = getattr(self.cfg, 'async_continue_file', None)
    ...
    def _run_spatio_temporal(self):
        ...
        try:
            ...
            # 检查是否是异步交互暂停（动态检查避免循环导入）
            if type(e).__name__ == 'AsyncInteractionPending':
                self.logger.info(
                    f"[Async] Optimization paused at generation {e.generation}. "
...
def main():
    ...
    parser.add_argument('--resume', type=str, default=None, metavar='CONTINUE_JSON',
                       help='Resume from async checkpoint using the specified CONTINUE JSON '
                            '(generated by generate_continue.py). '
                            'Example: --resume /data/results/signals/CONTINUE_task_gen10.json')
    ...
    try:
        ...
        # --resume：将 CONTINUE JSON 路径注入配置，供 create_scenario 传递给 NSGA2 main
        if args.resume:
            if not os.path.isfile(args.resume):
                print(f"Error: CONTINUE JSON not found: {args.resume}")
                sys.exit(1)
            optimizer.cfg.async_continue_file = args.resume
            optimizer.logger.info(f"Resume mode: CONTINUE JSON = {args.resume}")
        ...
        raise  # 透传 sys.exit()（包括 exit code 42）
    except Exception as e:
