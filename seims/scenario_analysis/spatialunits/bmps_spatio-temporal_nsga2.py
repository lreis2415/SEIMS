...
from scenario_analysis.spatialunits.scenario import initialize_scenario, scenario_effectiveness, \
    initialize_scenario_with_bmps_order, initialize_scenario_s_t, scenario_effectiveness_with_bmps_order
...
def main(scenario_obj):
    ...
    numpy.savetxt(scenario_obj.scenario_dir + os.sep + 'pareto_front_s_t_gen0.txt',
    ...
    for gen in range(1, gen_num + 1):
            ...
            for ind1, ind2 in zip(offspring[::2], offspring[1::2]):
                ...
                if random.random() <= cx_rate:
                    ...
                    if cfg_method == BMPS_CFG_METHODS[3]:  # SLPPOS method
                        toolbox.mate_slppos(ind1, ind2, scenario_obj.cfg.hillslp_genes_num)
        ...
        front = numpy.array([ind.fitness.values for ind in pop])
        # save front for further possible use
        numpy.savetxt(scenario_obj.scenario_dir + os.sep + 'pareto_front_with_s_t_gen%d.txt' % gen,
                      front, delimiter=str(' '), fmt=str('%.4f'))
