...
void gk_mcorePop(gk_mcore_t *mcore) {
    while (mcore->cmop > 0) {
        ...
        switch (mcore->mops[mcore->cmop].type) {
            ...
            default:gk_errexit(SIGMEM, "Unknown mop type of %d\n", mcore->mops[mcore->cmop].type);
        }
    }
...
void gk_mcoreAdd(gk_mcore_t *mcore, int type, size_t nbytes, void *ptr) {
    ...
    switch (type) {
        case GK_MOPT_MARK:break;

        case GK_MOPT_CORE:mcore->num_callocs++;
        ...
        case GK_MOPT_HEAP:mcore->num_hallocs++;
            ...
            break;
        default:gk_errexit(SIGMEM, "Incorrect mcore type operation.\n");
    }
}
