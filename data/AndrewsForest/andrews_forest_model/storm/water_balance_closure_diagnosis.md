...
当前 `storm/config.fig` 实际启用的是 `IKW_CH`，`CH_DW` 处于注释状态。因此本轮重点检查 `DEP_FS -> IKW_OL -> IKW_CH`。

为拆分坡面和河道格子的水量去向，`IKW_OL_balance.csv` 新增：
...
这些字段只用于诊断，不改变计算结果。no-change 运行结果仍为 NSE `-1.2878`、PBIAS `-43.64%`、峰值误差 `17.70%`。

### 当前参数下的连通瓶颈

当前 `param.cali` 中 `MANNING=8.0, VC`，这会直接替换坡面曼宁系数。该值远高于 wwj 分支报告中的 `MANNING=0.03`，属于极强坡面阻滞。

在 `MANNING=8.0`、GAR 恢复参数 `0.5 mm/h, 2 h, 0.2` 下：

| 窗口 | `DEP_FS` 新增地表水 | 到达河道格子上游入流 | `IKW_OL` 送入河道原始 QS | 窗口末非河道坡面滞水 | 出口局部峰 |
| --- | ---: | ---: | ---: | ---: | --- |
| 2 月 7 日事件 | `8.27 mm` | `1.78 mm` | `1.39 mm` | `6.27 mm` | `1.99 m3/s`，`2015-02-08 18:00` |
| 2 月 9-10 日 | `42.56 mm` | `31.81 mm` | `31.13 mm` | `16.05 mm` | `12.30 m3/s`，`2015-02-10 15:00` |

结论：2 月 7 日问题主要发生在坡面到河网格的连通阶段，大量水滞留在非河道坡面格子；不是 `IKW_CH` 把已经进入河道的 QS 吃掉。

### MANNING 控制实验

为确认坡面连通是否由糙率控制，临时覆盖 `PARAMETERS_SPEC` 中 `MANNING` 做控制实验；每次实验后已恢复 MongoDB 为 `MANNING=8.0`。

| `MANNING` | NSE | 主峰 | 峰现 | 2 月 7 日局部峰 | 2 月 9-10 日局部峰 |
| ---: | ---: | ---: | --- | ---: | ---: |
| `8.0` | `-1.2878` | `12.30 m3/s` | `2015-02-10 15:00` | `1.99 m3/s` | `12.30 m3/s` |
| `1.0` | `-3.0611` | `27.00 m3/s` | `2015-02-10 04:20` | `4.73 m3/s` | `27.00 m3/s` |
| `0.06` | `-4.0749` | `35.77 m3/s` | `2015-02-10 01:35` | `11.76 m3/s` | `35.77 m3/s` |
| `0.03` | `-4.1566` | `36.20 m3/s` | `2015-02-10 01:20` | `12.39 m3/s` | `36.20 m3/s` |

`MANNING=0.06` 约等价于 wwj 中 `MANNING=0.03` 与 `OL_SPEED_FACTOR=0.5` 的有效坡面阻力，因为 wwj 代码使用 `effective_n = MANNING / OL_SPEED_FACTOR`。

控制实验说明：

1. 降低坡面糙率确实能打通 2 月 7 日连通，2 月 7 日局部峰可以接近实测；
2. 但同一个常数糙率会把 2 月 9-10 日的大量地表水也快速送入河道，导致严重爆峰；
3. 因此当前问题不是简单“坡面流太慢”，而是“前期连通太慢 + 后期产流量太大”同时存在。

### 对 wwj 分支的含义

wwj 分支的较好结果并不是只靠 `IKW_OL` 的结构性连通修复。对比显示：

- wwj `IKW_OL` 主要新增 `OL_SPEED_FACTOR`，并对异常 `MANNING` 做兜底；
- wwj 最优参数报告中坡面 `MANNING=0.03`、`OL_SPEED_FACTOR=0.5`，有效阻力约 `0.06`；
- 直接在当前代码中使用等效 `MANNING=0.06` 会让 2 月 7 日变好，但 2 月 9-10 日爆峰，说明还必须同时控制后期地表水来源或把后期水转入壤中流/滞蓄。

下一步建议：

1. 不把 `MANNING=8.0` 作为最终参数；它物理量级不合理，只是在数值上压住后期爆峰。
2. 也不直接采用 `MANNING=0.03/0.06` 作为最终参数；它能修复 2 月 7 日，但会放大 2 月 9-10 日。
3. 下一步应回到 `SUR_SGA/IKW_IF` 的水量分配：在保持合理坡面糙率的同时，减少后期直接进入 `QS` 的地表水，增加近地表滞蓄、壤中流或事件间歇期入渗恢复。
4. 具体实验可用 `MANNING=0.06-1.0` 的合理范围，配合增强 2 月 9-10 日的 GAR 恢复或壤中流转化，而不是继续把 `MANNING` 单独调到极端值。
