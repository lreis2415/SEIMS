...
int netsetup(char *pfile,
    ...
    MPI_Init(NULL, NULL);
    {
        ...
        if (rank == 0) {
            float
                timeestimate = (1e-4 * TotalX * TotalY / pow((double) size, 0.2)) / 60 + 1;  // Time estimate in minutes
        ...
        for (j = 0; j < ny; ++j) {
            for (i = 0; i < nx; ++i) {
                ...
                if (!src->isNodata(i, j) && src->getData(i, j, tempShort) > 0 && !flowDir->isNodata(i, j)) {
                    ...
                    inext = i + d1[p];
                    jnext = j + d2[p];
                    if (!src->isNodata(inext, jnext) && src->getData(inext, jnext, tempShort) > 0
                        && !flowDir->isNodata(inext, jnext)) {
                            contribs->setData(i,
                                              j,
                                              (short) 1);//addData? What if it is a junction and has more floing into it?
                        } else {
        ...
        while (!finished) {
            ...
            while (!que.empty()) {
                ...
                for (m = 1; m <= 8; ++m) {
                    inext = i + d1[m];
                    jnext = j + d2[m];
                    if (pointsToMe(i, j, inext, jnext, flowDir) && !src->isNodata(inext, jnext)
                        && src->getData(inext, jnext, tempShort) > 0) {
        ...
        while (!finished) {
            ...
            while (!que.empty()) {
                ...
                for (m = 1; m <= 8; ++m) {
                    ...
                    if (pointsToMe(i, j, inext, jnext, flowDir) && !src->isNodata(inext, jnext)
                        && src->getData(inext, jnext, tempShort) > 0
                        && !idGrid->isNodata(inext, jnext)) {//!idGrid->isNodata(inext,jnext)){
                        inneighbors[k] = m;
                        //  stream orders are stored in lengths grid to save memory
                        nOrder[k] = (short) lengths->getData(inext,
                                                             jnext,
                                                             tempFloat); ///  Assign value//  Get the order from the link referenced by the value in idGrid and //   save it in nOrder[m-1]
                        k = k + 1;  // count inflows
                    }
                }
...
//  Block to swap links
            if (size > 1) {
                ...
                for (i = 0; i < nx; i++) {//todo fix this to add passed to queue
                    if (contribs->getData(i, 0, tempShort) == 0
                        && contribs->getData(i, -1, tempShort) != 0) //may receive flow from more than 1
                    {
                        t.x = i;
                        t.y = 0;
                        que.push(t);
                    }
                    if (contribs->getData(i, ny - 1, tempShort) == 0 && contribs->getData(i, ny, tempShort)
                        != 0)//only looks at one direction of flow, out of a partition && idGrid->isNodata(i,ny-1) && src->getData(i,ny-1,tempShort)==1
                    {
                        t.x = i;
                        t.y = ny - 1;
                        que.push(t);
...
//  Block to write link data
        ...
        if (rank == 0) {//open output point file
            ...
            createStreamNetShapefile(streamnetsrc,
                                     streamnetlyr,
                                     hSRSraster); // need raster spatail information for creating spatial reference in the shapefile
            ...
            for (ilink = 0; ilink < myNumLinks; ilink++) {//only once per link
                fprintf(fTreeOut,
                        ...
                        LinkIdU1U2DMagShapeidCoords[ilink][0],
                        LinkIdU1U2DMagShapeidCoords[ilink][1],
                        LinkIdU1U2DMagShapeidCoords[ilink][2],
                        LinkIdU1U2DMagShapeidCoords[ilink][3],
                        LinkIdU1U2DMagShapeidCoords[ilink][4],
                        LinkIdU1U2DMagShapeidCoords[ilink][5],
                        LinkIdU1U2DMagShapeidCoords[ilink][6],
                        LinkIdU1U2DMagShapeidCoords[ilink][7],
                        LinkIdU1U2DMagShapeidCoords[ilink][8]);
                //fflush(fTreeOut);
                ...
                for (ipoint = i1; ipoint <= i2; ipoint++) {
                    fprintf(fout,
                            ...
                            PointXY[ipoint][0],
                            PointXY[ipoint][1],
                            PointElevArea[ipoint][0],
                            PointElevArea[ipoint][1],
                            PointElevArea[ipoint][2]);
            ...
            for (i = 1; i < size; i++) {// send message to next process to wake it up
                ...
                MPI_Recv(&procNumLinks,
                         1,
                         MPI_INT,
                         i,
                         0,
                         MCW,
                         &mystatus);//get points one at a time and print them to file
                if (procNumLinks > 0) {
                    ...
                    for (ilink = 0; ilink < procNumLinks; ++ilink) {
                        ...
                        fprintf(fTreeOut,
                                "\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\t%ld\n",
                                treeBuf[0],
                                treeBuf[1] + numPointsPrinted,
                                treeBuf[2] + numPointsPrinted,
                                treeBuf[3],
                                treeBuf[4],
                                treeBuf[5],
                                treeBuf[6],
...
//		id1IO.write(xstart, ystart, ny, nx, idGrid->getGridPointer());
        ...
        for (j = 0; j < ny; ++j) {
            for (i = 0; i < nx; ++i) {
                ...
                if (!flowDir->isNodata(i, j)) {   //  Only put on queue to do if it is off the stream network
                    if (src->isNodata(i, j) || src->getData(i, j, tempShort)
                        < 1) //idGrid->isNodata(i,j))  //  Only if it is no data do we have to do anything
                    {
                        p = flowDir->getData(i, j, tempShort);
                        inext = i + d1[p];
                        jnext = j + d2[p];
                        if (src->hasAccess(inext,
                                           jnext))  //  Only do anything with cells for which we have access to downslope cell
                        {
                            if (src->isNodata(inext, jnext)
                                || src->getData(inext, jnext, tempShort) < 1) { //idGrid->isNodata(inext,jnext))
                                contribs->setData(i, j, (short) 1);  //  downslope cell not on stream
                                wsGrid->setToNodata(i,
                                                    j);  // overwrite grid value that may have been inherited from outlet not on stream
                            } else {
                        ...
                        == 0)  // Ordert = 0 indicates that all watersheds are to be labeled with the number 1
                    {
                        tempLong = 1;
        ...
        while (!finished) {
            ...
            wsGrid->share();
            for (i = 0; i < nx; i++) {
                if (contribs->getData(i, -1, tempShort) != 0
                    && contribs->getData(i, 0, tempShort) == 0)//only looks at one direction of flow, out of a partition
                {
                    t.x = i;
                    t.y = 0;
                    que.push(t);
                }
                if (contribs->getData(i, ny, tempShort) != 0 && contribs->getData(i, ny - 1, tempShort)
                    == 0)//only looks at one direction of flow, out of a partition
                {
                    t.x = i;
                    t.y = ny - 1;
                    que.push(t);
