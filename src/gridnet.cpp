...
//		printf("rank: %d, X: %lf, Y: %lf\n",rank,x[i],y[i]);
        ...
        if (rank == 0) {
            ...
            //fprintf(stderr, "This estimate is very approximate. \nRun time is highly uncertain as it depends "
        ...
        if (useMask == 1) {
            ...
            thresh = 0;  
            //  Here we have a partition filled with ones and a 0 threshold so mask condition is always satisfied
        }
        //Begin timer
        ...
        while (!finished) {
            while (!que.empty()) {
                ...
                if (flowData->isInPartition(i,
                                            j)) {   // DGT thinks this is redundant - nothing should be on queue that is not in partition - but does no harm
                    ...
                    if (maskData->getData(i, j, tempLong) >= thresh) {
                        ...
                        for (k = 1; k <= 8; k++) {
                            ...
                            if (sdir > 0) {
                                if (maskData->getData(in, jn, tempLong) >= thresh
                                    && (sdir - k == 4 || sdir - k == -4)) {
